...
- Sidebar 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/_Sidebar.md` が navigable entry を網羅的に参照しているか検証する。期待 slug 集合を `{tmpdir}` ファイルシステムから構築する：
  ...
  抽出前に、`{tmpdir}/_Sidebar.md` 本文からコード記法を除去する。コード内のリンク風記法は記法そのものの説明であり、実リンクではない。まずブロックレベルのコード記法（バッククォート3個以上、またはチルダ3個以上の run で開始するフェンスドコードブロック。閉じ側は同じ文字の run で長さが開始側以上である行。加えて4スペース/タブインデントのコードブロックと HTML の `<code>` / `<pre>` 要素）を除去し、そのあとインラインコードスパン（バッククォート N 個の run を、同じ N 個の run で閉じた範囲）を除去する。ブロックレベルを先に除去することで、フェンス自身のバッククォート run がインラインスパンの区切りと誤認されて無関係なテキストを飲み込むことを防ぐ。コード除去後の本文から `](<slug>)` リンク先を抽出し、`期待 − 参照` が空でなければ **STOP** して欠落 slug を名指しで人間にエスカレートする。この信号で wiki に push してはならない。sidebar drift は entry を追加した PR がナビを保守しなかったということであり、release sync は黙って自動修正する層ではない。

  理由：entry 作成 / rename のコミットは release タイミングから分離されて行われるため、release 間に sidebar drift が静かに蓄積する。sync は invariant を強制する自然な再帰チェックポイント。Dogfood（2026-05-21）：build-2026-05-20.1 sync で E-J と p / r / s / t / u が `_Sidebar.md` から欠落していたが検出できず、wiki commit `5e47a90` で手動回収した。コード記法除去は下記 Cross-reference 整合性アサーションと同じ理由で適用する（#1547）。

- Cross-reference 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/*.md` 内のすべての wiki 内部リンクが実在 entry を指していることを検証する。抽出前に、本文からコード記法を除去する。コード内のリンク風記法は記法そのものの説明であり、実リンクではない。まずブロックレベルのコード記法（バッククォート3個以上、またはチルダ3個以上の run で開始するフェンスドコードブロック。閉じ側は同じ文字の run で長さが開始側以上である行。加えて4スペース/タブインデントのコードブロックと HTML の `<code>` / `<pre>` 要素）を除去し、そのあとインラインコードスパン（バッククォート N 個の run を、同じ N 個の run で閉じた範囲）を除去する。ブロックレベルを先に除去することで、フェンス自身のバッククォート run がインラインスパンの区切りと誤認されて無関係なテキストを飲み込むことを防ぐ。解決集合を構築する：
  ...
  コード記法除外（#1547）：除去なしだとアサーション自身の仕様文が自己一致してしまう。本ファイルと `skills/operations-on-release/SKILL.md` はパターンをインラインコードスパン内の `](<x>)` 記法で説明しており、wiki entry `wiki-sync-sidebar-integrity-check.md` も同様である。これにより全リリースで恒久的な偽陽性 STOP が 3 件発生していた（2026-07-26、v1.19.11 / build-2026-07-26.3 で実測）。コード記法を除去した結果、実リンク 66 件はすべて解決し、真の未解決リンクはゼロだった。
