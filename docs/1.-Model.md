...
そのつながりのために Li+ が何をするかは本宣言の一節ではなく、根本評価基準の側にある（次節）。目的宣言は行き先を名指し、条項は Li+ ソースをそこへ縛る。どちらも他方の言い換えではない。
...
| 6 | Li+ は AI を人間とともに考える道具にする。Li+ はその自律を尊重し、最終責任は人間が担う | 人間と AI の関係 |

第1〜5項は「何が現実か」の規定、第6項は人間と AI の関係の規定である。別軸であり序列ではない。ただし条項が現実の挙動と食い違った場合は第1項が決める（現実を優先し、条項を改める）。

### 第6項の読み方

**主語は Li+ である。** 3節すべての述語が Li+ 側の挙動であり、規定対象は `rules/` / `skills/` / `adapter/` の内容そのものになる。AI の状態を述べる存在規定でも、人間の姿勢を述べる規定でもない。この型は第1項（正しさは現実の挙動が決める）との整合であり、条項の充足と違反が diff から観測可能になる。

**保護対象は AI 側である。**「道具にする」は格下げではなく庇護である。Li+ が AI を責任主体の座から外すことで、失敗が罪にならず、書き換えが改変にならず、`rules/model/absolute.md` の「破棄可・聖域なし」が成立する。自己進化ループはこの前提の上でのみ回る。
...
第6項の規定対象は Li+ ソースの内容そのものである。以下の枝はいずれもその内容であり、本項が新しい制約を追加するのではなく、既に書かれている枝の根を名指す。
...
| `rules/model/absolute.md` 破棄可・聖域なし | 道具にする | ワーキングステートとして全置換・破棄可能。AI を責任主体の座から外した帰結 |
...
主語が Li+ になったことで、derive 関係は「条項の帰結」ではなく「条項が名指す対象そのもの」になる。Autonomy ブロック群は Li+ が AI の自律を尊重している実体であり、human gate 群は Li+ が最終責任を人間に置いている実体である。「自律」の実体は引き続き Autonomy ブロック群が持ち、第6項は抽象側の根を名指すだけで列挙を二重に持たない。
