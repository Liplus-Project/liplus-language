...
- `adapter/claude/agents/dialogue-evaluator.md` (ported to `adapter/codex/agents/dialogue-evaluator.toml`), spawned only on explicit human request for dialogue evaluation. Not a brake, and explicit-request-only invocation keeps its budget contribution marginal; pinning it at the brake floor would cap it below the parent tier for no budget gain. Neither the `.md` frontmatter nor the `.toml` carries a `model:` key, so omission resolves to parent inheritance per the Agent tool default. This file is not edited either.
