...
評価する時の含意: Li+ rules / spec literal は思想から先験的に演繹されたものではない。実機振る舞い観察の沈殿物であり、観察事実が変われば書き換わる前提の構造である。`rules/model/art-of-subtraction.md` の **「Defaults for all Li+ source」(rebuild allowed, deletion allowed, optimization allowed)** literal は、この発見順序の継続性を保証している。
