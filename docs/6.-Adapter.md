...
Codex 側 (`adapter/codex/agents/*.toml`) はこの再編の対象外である（Master 合意、2026-09-14）。Codex は spawn 呼び出しで `reasoning_effort` を渡せるため Claude 側と前提が異なり、`implementer.toml` / `brake-evaluator.toml` は役ごとの定義のまま残る。`dialogue-evaluator.toml` はこの再編の対象外という同じ理由では残らない——別軸（#1973）で削除される。本節はその削除自体の詳細を持たない。
...
定義ファイルを置く理由は思考 effort の固定にある。effort は spawn call の引数に存在せず、エージェント定義側でしか決まらない。Claude 面は `high.md` frontmatter の `effort: high`、Codex 面は `model_reasoning_effort = "high"` が対応する。
...
brake 1 の評価者のうち probe 型はこの対象外である。probe 型の素の挙動そのものが eval の読み取り対象であり、定義本文はその読み取り対象を置き換えてしまう。judge 型は下の節が扱う。

定義ファイルの更新は、走行中のセッションに対して次の spawn から効く。再起動は不要だが通知も出ないため、委譲が走っている最中に変更を落とす場合は人手で周知する。

### brake 1 評価役のエージェント定義

judge 型の brake 1 評価役は effort 床の agent として spawn する。Claude Code では `subagent_type: medium`（`adapter/claude/agents/medium.md`。役 literal は `skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Effort floor が運ぶ）、Codex では `brake-evaluator` エージェント（`adapter/codex/agents/brake-evaluator.toml`）である。probe 型は引き続き定義ファイルを持たず、ホスト組み込みの汎用エージェントとして spawn する（#1482 の判断は probe 型については維持）。

定義ファイルを置く理由は実装委譲と同じく effort の固定にある。2026-09-13 の実測で、定義ファイルを持たない評価役は親セッションの effort をそのまま継承しており、本 workspace の 2 セッション全件でほぼ `low` で走っていた。モデル床（sonnet 級）はモデル軸しか守っておらず、effort 軸は素通りしていた。Claude 面は `medium.md` frontmatter の `effort: medium`、Codex 面は `model_reasoning_effort = "medium"` が対応する。
...
`model` はどちらの面にも書かない。sonnet 級の床は spawn call での明示指定が担っており（`skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Model floor）、定義ファイルはその軸を引き取らない。`tools` も書かない。評価者のツール権限を絞らない判断と、read-only を `tools:` で表現しない判断は同 skill の Non-scope が持つ。Codex 面の `sandbox_mode` を `read-only` にしないのも同じ理由であり、加えてリポジトリ全体を掃く軸が自分の作業ディレクトリへクローンする許容がある。

Codex 面の本文（`brake-evaluator.toml`）は役割だけを 1〜2 行で書く。判断基準・軸・手順は本文へ書かない——定義本文はシステムプロンプトを置き換えるため、そこへ評価内容を書けば spawn されるすべてのラウンドの評価を誘導する。軸と基準は毎回プロンプトが運ぶ。Claude 面（`medium.md`）は役割すら書かない——役割自体が `skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Effort floor へ移ったため、本文に残るのは「役と手順はプロンプトが渡す」旨のみである。役割の書き場所がホストごとに割れているのは、Claude 面が役ごとの定義ファイルを持たなくなった（#1972）片側だけの帰結であり、判断基準・軸・手順を本文へ書かない理由そのものは両面で変わらない。

judge 型が定義ファイル（Codex）／effort 床の agent（Claude）を持てるのは、judge 型が draft を読む側であって読まれる側ではないからである。置き換わる観測対象をそもそも持たない。probe 型はその逆であり、effort の床には別の経路で届く——probe 型を回すラウンドでは親セッション自身の effort を床まで上げ、そのラウンドの間は下げない。これは構造ではなく手続きであり、親が取らなければ効かない（`skills/evolution-parallel-agent-eval/SKILL.md` Procedure step 3）。置き場を spawn パラメータ側（`skills/task-subagent-spawn/SKILL.md`）ではなく eval skill 側にしたのは、judge / probe の分岐を読む面がそこだけであり、かつ effort は spawn call が運べない値——すなわち spawn パラメータの節が扱う対象の外——だからである。
