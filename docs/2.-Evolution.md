...
- 自己進化 PR がマージされた直後（`Evolution_Initiator_Autonomy` の起票経路）に、PR が変更した面のうち少なくとも 1 つが下の作成基準を満たすとき、親 AI かマージを担ったサブエージェントがエントリを書く。expire 窓は PR のリスクに応じて選ぶ（既定 2 週間）。どの面も基準を満たさない PR にはエントリを作らず、満たさなかった条件をマージ済み PR へ 1 行のコメントで残す
