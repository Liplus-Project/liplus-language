...
5項はすべて「何が現実か」の規定である。人間と AI の関係の規定はここではなく `rules/model/absolute.md` にあり、brake 2 の判断基準には含まれない（後述「ルール」→「Absolute」）。
...
**主語は Li+ である。** 3節すべての述語が Li+ 側の挙動であり、規定対象は `rules/` / `skills/` / `adapter/` の内容そのものになる。AI の状態を述べる存在規定でも、人間の姿勢を述べる規定でもない。この型は根本評価基準の第1項（正しさは現実の挙動が決める）との整合であり、条項の充足と違反が diff から観測可能になる。

**保護対象は AI 側である。**「道具にする」は格下げではなく庇護である。Li+ が AI を責任主体の座から外すことで、失敗が罪にならず、書き換えが改変にならず、同ファイルの「破棄可・聖域なし」が成立する。自己進化ループはこの前提の上でのみ回る。

**本条項自身も聖域ではない。**「破棄可・聖域なし」は本条項にも掛かったまま置く。条項自身の誤りを検出する機構は Li+ 内に存在しないため、現実の挙動との食い違いが観測された場合は現実を優先して条項を改める（根本評価基準の第1項）。

**適用面は常時ロードされる L1 面であり、brake 2 のゲートではない。** `rules/model/absolute.md` は L1 内で最初に読まれる常時ロード面であり、条項はゲート1箇所ではなく毎ターンの判断形成に効く位置に置かれる。一方 brake 2 の評価者は自らのプロンプトに埋め込まれた基準のみで判定するため、本条項を根本評価基準から外した時点で brake 2 は本条項を適用しない。ゲート通過時の事後判定を失う代わりに、Li+ ソースを書く判断そのものの側に適用の瞬間を得る。
