...
- latest フラグは人間専用。実機検証後に `gh release edit {tag} --latest=true` で人間が flip する（**判断 ↔ 実行軸**：「人間専用」「人間が flip する」は**判断主体**を指す表記であって CLI 実行主体ではない。Master が判断し、明示的な go-sign 後に AI が gh CLI を実行する。詳細は本ドキュメントの「Master 判断ゲート」節）
...
> **判断 ↔ 実行軸**：上表の「人間のみ」は**判断主体**を指す。CLI（`gh release edit ... --latest=true`）を打つのは AI で、Master が明示的な go-sign を出してから AI が実行する。詳細は本ドキュメント「Master 判断ゲート」節を参照。
...
#### Master 判断ゲート（判断 ↔ 実行軸の分離）

(→ `rules/operations/execution-mode.md` の `Master judgment gate` 節)

以下の操作は **Master 判断ゲート**の対象。release create / Latest flip / force push / tag delete / マージ済み PR delete / main ブランチへの破壊的変更 / 公開済み artifact への破壊的変更。これらのゲートは**判断権限**に対するものであって、**実行権限**に対するものではない。

- Master が yes / no を判断する。
...
- AI の返答内で Master に gh CLI を打つように指示しない。CLI は AI が打ち、Master は go-sign を出す。

ゲート対象操作に対する Master の発話が曖昧な場合は、**最も保存的な解釈をデフォルト**として採る。先行する go-sign を別ゲートに自動拡張しない（release create の go-sign は Latest flip の go-sign ではない）。
