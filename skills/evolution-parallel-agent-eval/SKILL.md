...
3. **Parallel subagent spawn** - Select N, M, P per draft nature and spawn in parallel. Default `N=3, M=all axes, P=1`, total invocation = 3; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   - the shape that comment is written in: findings at full length with a verbatim quote and its `path:line`, an axis with no finding at one line, no echo of the criteria this prompt supplies, and the prose in the language named here, with quotes left in the source's own language (see Report shape, Language). That sentence is what the prompt carries; the Report shape section behind it is the parent's reference, not prompt payload, since pasting the section would put back into the prompt the bulk this contract removes. The shape is a contract on the comment, so leaving it to the evaluator's discretion is what puts every axis at finding length
...
6. **Judgment** - Actor = the resumed implementation subagent. consistent -> nothing to apply; report back. partial / negative -> adjudicate each finding against the source, answer it on the PR with an accept or a reject and the reason (answer shape = Report shape), apply what was accepted, commit, push, and stop again at CI green. The parent then self-reviews the resulting thread. Or abort. **Single round**: steps 2-4 produce one verdict per draft, and the revised draft ships without re-verification (see Non-scope: what the single-round cap gives up). The author's response-and-revision pass is the tail of that one round, not a second one. The cap does not block step 2's `rules/*` retry path, and that path only: there, a refused apply leaves the round with no verdict, so re-running from a session that can apply IS the round, not a re-verification of it. Do not generalize this to any round that failed to produce a verdict — an evaluator crash, a malformed prompt, or a timeout does not earn a fresh round. Its other two branches (`skills/*` direct-Read fallback, `rules/*` proceeding on a reduced-confidence deviation record) produce a verdict and are unaffected
...
Fixes the form of the brake 1 comment thread — the evaluator comment at Procedure step 3 and the author's answers at Procedure step 6. Both are asymmetric on the same seam: the side carrying a finding is written at full length, the side nobody contests at one line. Left unfixed, the parent composes the form per run and every axis is answered at finding length. What this section fixes is delivery: it does not change which axes are asked (Trigger, Axis selection), how many evaluators answer them (Constraint: N=1 prohibited, minimum N=3), or how the answers aggregate (Design Dimensions).

Scope = the brake 1 path. brake 2 is out: its evaluator has its own prompt file and returns a verdict inline (`adapter/claude/agents/l1-gate-eval.md`). On the other Trigger entries there is no PR surface, and the asymmetry still governs what the evaluator writes — the reasoning below turns on who adjudicates a line, not on where the line is posted.

### Evaluator comment

- **Axis with a finding**: the verbatim quote of the literal at issue, its `path:line` at the named SHA, and why it is a defect. Full length, and not a compression target — the author adjudicates against that quote and would otherwise re-fetch the source once per finding, and a defect asserted without hitting the literal is the shape `rules/model/trigger-check-gate.md` Literal check and the `#1673` cluster stand against.
...
- **Reject**: full. The reason stands on the PR next to the finding it rejects, and under the single-round cap nobody re-argues it, so this is the surface the parent reads at Procedure step 7.
- **Accept**: one line — which finding, and what changed. The change itself is already externalized in the diff and the commit; prose restating it is a second copy of the same content for the parent to reconcile against the first.

### Language

The comment's prose language is the one part of the shape discretion cannot reach. The evaluator holds the contract but not the value it resolves to: `CLAUDE.md` and `rules/**` reach subagent context in full while hook firing output does not (Constraint: Character_Instance non-inheritance), and the resolved language values are exactly what the session-start hook emits. The contract's own instruction to apply those values is therefore unexecutable where the evaluator reads it, so absent a named language the only language on its surface is the prompt's own, and that is what it writes in. On 2026-08-08 all three evaluators at PR #1701's brake 1 did exactly that, posting findings in the prompt's language rather than the one the contract resolves for a PR comment in that workspace. The author's answers and the parent's self-review on the same PR landed in the resolved language instead, and not because either actor resolved the contract unaided: the parent holds the emitted values in its own session and writes the self-review itself, and the implementation delegation prompt already carries a standing language item (`skills/task-subagent-prompt/SKILL.md` Delegation prompt hygiene) that the evaluator prompt had no counterpart of. That asymmetry is what scopes this to the evaluator side — the author's answers are covered where that item already lives, and restating it here would be a second copy of it, the second copy being what drifts.

Resolution: the language `Workspace_Language_Contract` (`adapter/claude/CLAUDE.md` / `adapter/codex/AGENTS.md`) resolves for a PR comment, a surface that contract's own definitions name. It is the single source, so neither its precedence nor a resolved language name is written here; the name is the worse of the two copies, this file shipping to every workspace while the value is per-workspace. What belongs here is only that the parent resolves the value before the spawn and names it in the prompt (Procedure step 3), instead of letting the prompt's own language stand in for a contract nobody consulted.

Quoted source text is excluded, by the property the finding rests on rather than for convenience: the author adjudicates against the literal, and a translated literal is not the literal. A verbatim quote and its `path:line` stay as the source has them — Li+ source is English (`rules/model/liplus-coding-rule.md` Source Language), so in a workspace that resolves to any other language a finding is mixed by construction, the quote in the source's language inside prose in the resolved one. Left unstated, the pair collapses one way or the other: the quote gets translated with the prose, or the prose stays in the quote's language. A clean axis carries no quote, so its one line is prose and a pointer, and a pointer is not prose.

Nothing is named for a report that returns into parent context instead of landing on a PR: brake 2, and the Trigger entries with no PR surface. That criterion is location, which the scope paragraph above rules out for the rest of this section, so the two subjects part company here rather than sharing a reason — shape turns on who adjudicates a line wherever it is posted, while language turns on whether the text itself reaches a contract-governed surface. On those Trigger entries the asymmetry keeps governing what the evaluator writes and no language is named, because what eventually reaches the human is the parent's own writing and the parent resolves that as it already does. brake 2 is out of this section on its own terms either way.
