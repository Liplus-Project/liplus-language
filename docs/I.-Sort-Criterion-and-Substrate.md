...
- `skills/model-no-safety-net/SKILL.md` / `skills/model-pair-review/SKILL.md` / `skills/model-requirement-deepening/SKILL.md` / `skills/model-review-output-partition/SKILL.md` / `skills/model-web-search-judgment/SKILL.md` / `skills/model-agentic-search/SKILL.md` — 対応する rule が `rules/model/` に存在しない独立 skill。発火モーメントを持つため真の skill。現状維持。`model-agentic-search` は #1220 で新設 (calibration + category OR trigger、 broad 探索軸の mechanical core)、 `model-web-search-judgment` は同 PR で Web 側消費規律のみ残す形に縮小。
