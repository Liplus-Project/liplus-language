...
1. **ローカルプロトタイプ**（issue・ブランチ無し）で設計を固める
2. 固まったら **issue を作成**する
3. 要求仕様書（`docs/`）を変更内容に合わせて **更新する**
4. 実装とテストを **コミットする**
5. docs と実装を **同一PR** に含めて提出する

PR 内のコミット順序は問わない。ただし docs と実装が同一 PR に含まれていなければならない。

挙動変更・バグ修正・仕様変更では、要求仕様書の該当箇所をコードと同じ PR 内で更新する。
コードとテストは、その仕様差分を実装・検証するために更新する。

**理由：** ドキュメントと実装を同一PRに束ねることで、コードは仕様の実装になり、テストは仕様の検証になる。リリース時にソースコード・仕様書・テストを同じバージョンで束ねられる。
