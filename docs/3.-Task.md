...
（→ `skills/agentic-search/SKILL.md`。L1 Model Layer。トリガー時ロード）

#1380 (Phase 3 of #1217) で、旧 4 skill (`skills/model-agentic-search/` 機械的 core + `skills/model-web-search-judgment/` Web 側消費規律 + `skills/task-research-strategy/` 親 AI governance + `skills/task-retrieval-orchestration/` 親 AI 消費規律) を単一 skill `skills/agentic-search/` に encapsulate 済。 auto-invocation 面が単一化、L3 task layer 側に残置する独立 skill は無くなった。
...
詳細は `skills/agentic-search/SKILL.md` 本体を参照する。
...
| Agentic Search (#1380 統合; 旧 Research Strategy + Retrieval Orchestration を吸収) | `skills/agentic-search/SKILL.md` | L1 | skill トリガー |
