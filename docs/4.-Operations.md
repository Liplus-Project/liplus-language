...
- `CONFLICTING` → まず同じ非破壊的な取り込み `gh pr update-branch {pr} -R {owner}/{repo}` を試行。成功なら CI ループを step 1 から再開。マージ競合で失敗した場合は issue コメント → 人間にエスカレーション。ここで rebase + force push にフォールバックしない。force push は無条件の human 判断ゲート項目（後述の「人間確認が必要な操作」）であり、AI 側で進められる経路が残らない。
