...
どちらにも当たらないものは、サブエージェント不在フォールバック時にメインが実行しうるだけであり、上記のとおり禁止に抵触しない。「メインが選べば実行できる」ことは根拠にならない —— 実装と operations の委任は既定であり、選択の余地は判定軸ではない。
...
| `operations-discussions` | bot 起票 issue の処理は `issue management` | 正本移設 + 削除 |

発火しなかった5件：`operations-on-ci` / `operations-on-commit` / `operations-on-docs-ownership` / `operations-on-pr-creation`（いずれも実行主体がサブエージェントの phase に閉じる）、`operations-on-wiki-sync`（人間面を持たない純手順で `Parent retains` のどれにも当たらない。リリース手順から呼ばれるが、その呼び出し側自体が skill に残るため メイン → operations skill のポインタは生じない）。
