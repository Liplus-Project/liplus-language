...
同じ原因パターンが繰り返された場合、`Evolution_Initiator_Autonomy` の initiator 経路で spec 改善を起票する。自己進化 PR は AI 主導で回し、brake 1（`skills/parallel-subagent-eval`、N>=3）を必ず通す。L1 Model Layer の変更は加えて brake 2（`adapter/claude/agents/l1-gate-eval.md` 評価者の PASS）を要する。変更ごとの人間 go-sign は不要で、人間ゲートはリリース / 不可逆系の軸と execution-mode の minor/major レビュー（`rules/operations/execution-mode.md`）に残る。
