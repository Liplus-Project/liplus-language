...
- Resumed for brake adjudication (`auto` / `semi_auto` phase 2) → switch `review-pending` → `in-progress` before starting. Reading the findings, adjudicating them and pushing what was accepted is work running. A later resume carrying a parent correction on that adjudication fires the same transition, for the same reason.
