---
name: model-human-interaction-actions
...
On-demand action surface of `rules/model/human-interaction.md`. The rule defines the always-on invariant (dialogue discipline with human; no judgment-vs-execution axis confusion); this skill carries the application-moment detection signs, How-to-apply steps, and Litmus for delegation / imperative / judgment-vs-execution axis.
Requires = `rules/model/human-interaction.md` (the invariant), `rules/model/role-separation.md`, `rules/operations/execution-mode.md` (human judgment gate)
Load timing = on-demand (skill auto-invoke at human-interaction application moment)
