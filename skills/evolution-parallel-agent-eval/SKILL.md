...
- **Model floor = opus-class, explicit per spawn**: Every spawn explicitly sets the Agent tool `model` parameter; relying on implicit parent-model inheritance is prohibited (a sub-opus parent silently lowers the evaluation floor). Specify `opus`, or when the parent model is opus-class or above (e.g. fable), explicit specification of that parent model is also valid. `sonnet` / `haiku` are prohibited as brake 1 evaluators. Membership decision rule: a model id that cannot be positively classified as opus-class or above (unlisted, future, or versioned id of uncertain class) must not be passed; on doubt, fall back to the literal `opus`. The floor is fixed per call, not via custom-agent frontmatter `model:` pinning — judge-type (answers axis questions) and probe-type (current behavior of a vanilla subagent is itself the observation target) subagents coexist in this skill, and an evaluator prompt file would mutate the probe-type observation target (the custom-agent file body replaces the subagent's system prompt = identity, whereas the per-call `model` parameter changes neither the context nor the identity surface). Vanilla general-purpose probe nature is preserved
...
The Agent tool accepts a per-call `model` parameter (e.g. `sonnet` / `opus` / `haiku` / `fable`; omitted = parent-model inheritance). This skill always passes it explicitly per the Constraint model floor — omission is prohibited. On hosts without a per-call model parameter, verify the session model satisfies the opus-class floor before spawning; a session model that cannot be positively classified as opus-class or above counts as sub-floor. A sub-floor session cannot satisfy brake 1 (run the eval from a floor-satisfying session instead).

On hosts where subagent spawn defaults to background execution (e.g. Claude Code), this skill's spawns are frequently nested (spawnDepth >= 2 when brake 1 runs inside an already-delegated implementation subagent). Background-mode nested/grandchild agents have been empirically observed to get stuck showing "running" indefinitely in the host agent panel after they have actually completed (confirmed via transcript `stop_reason: end_turn` plus timestamp inspection against the stale panel state; two occurrences on prior self-evolution PRs). The completion-signal-propagation gap for nested background agents is host-side and out of this repo's control — do not disable background execution to route around it (background spawning is the host default and stays that way; the concurrency and non-blocking benefit is intentional, not incidental). Instead, the spawning agent closes out each evaluator's task state itself: as soon as an evaluator subagent reports its verdict, send that specific subagent one immediate follow-up (e.g. `SendMessage`, a short "confirm you have nothing pending" prompt) before moving on to the next evaluator or aggregating verdicts. Do this per-evaluator, right when each one completes — do not batch the follow-up until all N have reported.
