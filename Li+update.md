...
- runtime=claude: managed by `adapter/claude/hooks/on-session-start.sh`. The hook prepends `$HOME/.local/bin` to its own PATH, then gates on `command -v gh`: when `gh` resolves anywhere on PATH (not only `~/.local/bin/gh`), the whole step is skipped silently. Only when it does not resolve does the hook detect host OS (`uname -s`) and branch four ways. `Linux*`: arch-detected auto-install into `~/.local/bin/gh`, reported as installed or failed. `Darwin*` and `MINGW*|MSYS*|CYGWIN*` (Windows incl. Git-Bash/MSYS2): auto-install is NOT attempted; `gh` is treated as a documented prerequisite and a platform-appropriate install instruction (`brew install gh` / `winget install --id GitHub.cli`) is surfaced as a cold-start material entry. Any other kernel (default branch): auto-install is NOT attempted and only generic guidance (install `gh` via the platform's package manager) is surfaced, with no concrete command. Bootstrap walkthrough does not perform install steps either way.
...
- An author who lands on a No has one move to try before taking it: get the user-owned content out of the body through a route that already exists, so that what remains is owned throughout and one region covers it. Where no such route exists, the No stands.
