...
Li+core.md と Li+operations.md は `.claude/rules/` に配置され、常時コンテキストに存在し compaction を生存する。Li+github.md は `.claude/skills/` に配置され、skill auto-invocation で必要時に読み込まれる。手動再読込は不要。
...
**エントリーポイント（Li+agent.md）：** ホスト指示ファイルへの Li+ 注入を担う。読込順序、再読込トリガーのマッピング、Character Instance 配線、workspace 言語契約の配線を所有する。rules/skills 経由の宣言的読込と、hook 経由の focus pointer を併用する。

**ランタイムバインディング（Li+claude.md 等）：** ホスト環境固有のトリガー実装を担う。エントリーポイントが定義するトリガー契約を、ランタイム固有のメカニズム（rules/、skills/、hook 等）へコンパイルする。

---

## Claude Code バインディング

Li+claude.md に Claude Code 用の hook 定義と rules/skills 生成テンプレートを格納する。bootstrap 時に3種類の生成物を作成する：

1. **rules/ ファイル：** Li+core.md と Li+operations.md を `.claude/rules/` に配置する。YAML frontmatter（`alwaysApply: true`）を付与し、常時コンテキストに存在させる。compaction を生存する。
2. **skills/ ファイル：** Li+github.md を `.claude/skills/li-plus-github/SKILL.md` に配置する。skill frontmatter で auto-invocation タイミングを宣言する。Issue Operations セクションは Li+operations.md に移動済みのため含まない。
...
動作：no-op。Li+core.md と Li+operations.md は `.claude/rules/` で常時コンテキストに存在し、compaction を生存するため再注入は不要。forward compatibility のために hook ファイルは残す。
...
| `gh issue create` / `gh issue edit` | オペレーションレイヤーの Issue Format + Sub-issue Rules セクション + Milestone Rules focus pointer（skill） | — |
...
└── .claude/
    ...
    │   └── Li+operations.md   # 常時コンテキスト（alwaysApply: true）
    ├── skills/
    │   └── li-plus-github/
