...
#### 起動時ステータスマーカー（gh install / Li+ update status）

cold-start 素材収集より前に、hook は `register_section` を経由しない printf 直接出力で2つのマーカーを emit する。diff-only 比較（sha256 fingerprint 方式）の対象外であり、`startup` / `resume` / `clear` / `compact` いずれの matcher でも毎回再 emit される（matcher 分岐は後段の cold-start 素材にのみ適用される）。

- **`━━━ gh install ━━━`**：`gh` CLI が PATH 上に無い場合のみ出力。Linux ホストは `~/.local/bin/gh` へのアーキテクチャ判定つき自動インストールを試み、成功時は無言スキップ相当（マーカーは出さない）。macOS / Windows(Git-Bash・MSYS2・Cygwin) / その他ホストはドキュメント化された前提条件として扱い、自動インストールせずプラットフォーム別の導入コマンドを案内する。`GH_INSTALL_STATUS=installed` / `failed: <末尾ログ抜粋>` / `missing: <ホスト別ガイダンス>` の3値。`liplus-language` リポジトリ未解決（bootstrap 前）の段階では、install が失敗または未導入の場合にこのマーカーのみを出力して hook を終了する（cold-start 素材収集フェーズへ進まない）。
...
出力順序（bootstrap 済みの通常セッション）：`Li+ update status` → `gh install`（該当時のみ。install を試行した結果が installed / failed / missing のいずれであっても出力）→ cold-start 素材（matcher 依存）。
