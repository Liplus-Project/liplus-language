...
**他 surface との分離：** cluster tally の expire / 閾値削除は `rules/evolution/promotion-judgment.md`、memory ↔ docs ティアリングは `skills/evolution-persistence-tiering/SKILL.md`、self-evaluation の採点軸は `rules/evolution/self-eval-axes.md` に書く。本セクションは memory 内部の entry 書式と運用のみを射程とする。
