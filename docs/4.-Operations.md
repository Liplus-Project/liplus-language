...
呼び出しアンカーは `rules/operations/main-agent-procedures.md` の `## Merge Execution` がマージの瞬間に名指す。その瞬間が各モードで何であるかは同節の Post-merge moment が定める —— `auto` / `semi_auto` では親自身のマージ、`trigger` ではマージの瞬間に立つエージェントが居ないため、メインエージェントがマージを最初に観測したターンである。この手続きの実行主体はどのモードでもメインエージェントであり、operations 系 skill はメインには発火しないので、アンカーをそこに置くと読み手を素通りする。
...
2. `head_branch` が `main` → `gh run view <id> --json displayTitle` を読み、**自セッションが書き込みを行った**対象かを照合する（`event=issues` は issue タイトル、`push` は commit タイトル、`pull_request` は PR タイトルを運ぶ）。「書き込みを行った」は起票に限らず書き込み全体を指す。issue body を編集しただけのセッションも、起票したセッションと同様にその題名に一致する。この手順が問うているのは、その run が自セッションの行った操作の到達確認かどうかであり、issue の起票者ではない
