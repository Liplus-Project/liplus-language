...
クローズされた issue の本文に PR 単体では満たせない完了条件（`rules/operations/main-agent-procedures.md` の `## Issue format`）があり、その条件が未達のときは、マージを実行した agent がマージ直後に issue を reopen し、条件が満たされるまで開いたままにする。
