...
- `Evolution_Initiator_Autonomy`（`adapter/claude/CLAUDE.md`）は、self-evolution PR が L1 Model Layer ソースを touch するとき、この観測ゲートの上に根本基準評価の要件（brake 2）を重ねる。二つのゲートは直交する軸である
...
**フック連携の前提：** `on-session-start.sh` フックがセッション冒頭で、直近リリースタグ・判断構造索引の先頭・自己評価ログの先頭・cold-start ルール本文を表面化する。build-2026-05-11 以降、`startup` matcher では前セッションから変化のあった section のみを emit する diff-only 出力に変更された（context 消費削減）。cold-start ルール本文だけは drift recovery anchor として常時 emit され、diff 比較対象から外れる。
...
| no-new-material marker | 全 section 不変、かつ observation surface が emit されていない | "No new orientation material since last session" 1 行 + cold-start rule literal |
...
- **指摘を裁くのは親ではなく、実装した subagent を再開したもの（著者）である。** brake 1 の評価者は指摘を PR コメントへ直接投稿し、親への戻りは投稿したコメントの URL 1行に絞る。親は評価者を spawn し、著者を再開し、セルフレビューし、マージする——指摘そのものは親の文脈を通らない。再開した著者は PR コメントを読み、各指摘に受入または却下を PR 上で答え、受け入れた分を修正して再び CI green で止まる。親はそのスレッドを入力にセルフレビューする。効く点は二つで、却下された指摘が却下理由の隣に名指しで残るため単巡打ち切りの下で誰も再反論しない弱点が親の読む面に現れること、および再開が実装文脈を保つため `skills/task-subagent-delegation/SKILL.md` が受容していた「親が報告から文脈を組み直す」対価が支払われなくなること（「実装は常に委譲」の判断軸である規則の単純さは不変で、条件分岐が1つ減る）。エージェントの深さは 1 のままで、評価者を spawn するのは親であり著者は何も spawn しない。正本は `rules/evolution/initiator-autonomy.md` Two-stage brake の Adjudication actor
- brake 1 の評価者へは **評価対象を書き換えるな** と委譲プロンプトで明示する。文言は `skills/evolution-parallel-agent-eval/SKILL.md` Constraint に literal として置いてあり、親はそれを写す（毎回作文しない）。上の材料に親のクローン内のパスを含めないのはこの指示と対であり、前者が書き込みの意図を断ち、後者が共有ベースラインという的そのものを外す。PR コメントの投稿はこの禁止に当たらず、literal の内側で明示的に許してある——コメントは対象についての記録であって対象そのものではなく、同時に走る他の評価者が読む面を1バイトも動かさない（commit は動かす）。評価者のツール権限は絞らない（custom-agent の `tools:` は Claude Code では本文が system prompt を置換するため identity ごと変わり、probe 型の観測対象を壊す）。したがってこれは構造ではなく親が思い出す手続きであり、忘れれば効かないことを受容している。brake 2 は `adapter/claude/agents/l1-gate-eval.md` が `tools: Read`、Codex 版が `sandbox_mode = "read-only"` を持ち、材料も inline 渡しでリポジトリを名指ししないため対象外
...
- brake 1 のコメントスレッドの**形**は skill 側で固定してあり、親が毎回作文しない。所見のある側は full、誰も争っていない側は 1 行という非対称である。評価者コメントは、所見のある軸＝逐語引用＋`パス:行`（名指しした SHA 時点）＋なぜ欠陥か／所見の無い軸＝1 行（軸名＋その軸の語での判定＋根拠の `パス:行`。リポジトリ全体を掃く軸は指せる 1 行が無いため、再実行可能な形の掃き出し（パターンと掃いたパス）とヒット数で代える）。著者の回答は、却下＝full（単巡打ち切りの下で誰も再反論しないため、これが親が step 7 で読む面そのものである）／受入＝1 行（何が変わったかは diff と commit に既に外部化されている）。両側で禁止するのは、親がプロンプトで渡した基準・閾値・軸文言の復唱である。所見側の逐語引用は削減対象ではない——著者は引用に当てて裁定するため、削るとソースの取り直しが指摘ごとに発生し、リテラルに当てずに断定する形（`rules/model/trigger-check-gate.md` Literal check / #1673 cluster）へ戻る。所見の無い軸で「見て clean と言った」を支えるのはポインタの**解決可能性**であり、名指しした SHA で開けること、開いて判定と合わないなら 1 回の参照で露見することがその働きである。掃引軸には開く先が無いため、その働きは再実行可能な形で述べた掃き出しが担う（不在の主張は開いてではなく再実行で検査され、再現しないヒット数は解決しないポインタと同じ形で落ちる）。残余（誰も開かない clean 軸のポインタ、および判定に合うよう事後選定されたポインタ）は post-merge observation 軸で受容する。コメントの散文の**言語**は、この形のうち評価者の裁量が届かない部分である——評価者は契約を持つが、解決後の値を持たない。`CLAUDE.md` と `rules/**` は subagent context へ全文届くが hook の発火出力は届かず（同 skill の Constraint: Character_Instance non-inheritance）、解決後の言語値はまさにその session-start hook が出すものである。したがって「その値を適用せよ」という契約自身の指示は評価者が読む場所では実行できず、名指しが無ければその面に在る言語はプロンプト自身の言語だけになり、それで書く（2026-08-08、PR #1701 の brake 1 で 3 体ともそうなり、契約がその workspace の PR コメントに解決する言語ではなくプロンプトの言語で投稿した。同 PR で著者の裁定と親のセルフレビューが解決後の言語に着いたのは、どちらかの主体が契約を独力で解決したからではない——親は自分の session へ出力された値を持ちセルフレビューは親自身が書くものであり、実装の委譲プロンプトには言語の常設項目が既にある（`skills/task-subagent-prompt/SKILL.md` Delegation prompt hygiene）。評価者プロンプトにはその対応物が無かった。この非対称ゆえに規定は評価者側だけに置く——著者の回答は既にその項目が在る場所で覆われており、ここへ書き直せばその写しになる）。解決先は `Workspace_Language_Contract`（`adapter/claude/CLAUDE.md` / `adapter/codex/AGENTS.md`）が PR コメントに解決する言語であり、precedence も解決後の言語名も skill 側へ写さない（skill は全 workspace へ配布され値は workspace ごとであるため、言語名の方が悪い写しである）。親が spawn 前に解決してプロンプトへ名指しする、それだけが skill 側の規定である。**逐語引用は対象外**であり、根拠は都合ではなく所見が乗っている性質そのもの——著者は引用に当てて裁定するため、訳された literal は literal ではない。Li+ source は英語（`rules/model/liplus-coding-rule.md` Source Language）であり、他の言語へ解決する workspace では所見は構造的に混在する（引用は原文の言語のまま、地の文は解決後の言語）。明示しなければ引用まで訳されるか地の文まで引用の言語へ倒れるかのどちらかになる。PR に着かず親の文脈へ戻る報告（brake 2、および PR 面を持たない Trigger 項目）には何も名指ししない。この基準は投稿先という位置であり、Scope 段落が本節の他の部分については退けている基準そのものである——ゆえに二つの主題はここで理由を共有せず分岐する。形は「誰が裁くか」に乗り投稿先を問わないが、言語は「その本文自体が契約の及ぶ面へ出るか」に乗る。PR 面を持たない Trigger 項目では非対称が governs し続けたまま言語は名指ししない（人間に届くのは親自身が書く本文であり、親はそれを既にそうしているとおり解決する）。brake 2 はいずれにせよ自前の理由で本節の外である。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Report shape
