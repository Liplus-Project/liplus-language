...
Composing a set of axes per draft is what the two held axes replace on that path, and a draft that looks unlike the last one is not a reason to compose one: what the parent was writing per target was these same two questions, and A and B are that set read on its form rather than on its subject matter (#1832). Do not add a third per-draft axis, and do not split either of the two.

The fixation reaches the brake 1 path and stops there, and what stops it is the held pair's own `Basis`: both resolve against a PR and the issue that PR closes, which a draft converged outside a PR flow has neither of. At the other Trigger moments above the fixed axis is included as always, and any further axis is composed per draft nature and written under Axis statement form, as the whole set was before the pair was held. Only the brake 1 path was measured (#1832), so the fixation is not carried to moments the measurement did not reach.
...
Why these two, and why held: two measurement cycles settled the count at two — neither one axis nor six (#1832). Splitting B by target has an obvious shape to it — one axis per file, or per artifact class — so it is available at every run, which is why the prohibition sits in Trigger, Axis selection where the axes are picked rather than only here. Judgment record: `brake1-per-draft-axes-held-at-two` (both cycles, the seam-loss result that rules out splitting B, and why B alone does not reach what A asks).
