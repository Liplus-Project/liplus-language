...
| 課題 | 対応する装具 / 仕組み |
|------|-----------------------|
| 先走り | rules (制約装具)、Master 判断 gate (`rules/operations/execution-mode.md`)、Expansion Limit (`rules/model/expansion-limit.md`) |
...
- Codex adapter は dialogue 意図 reading (humane register / Master 真意) の維持を強める
...
これにより、Master の workspace 言語契約 (`LI_PLUS_BASE_LANGUAGE` / `LI_PLUS_PROJECT_LANGUAGE`) と独立して、AI 内部実行の精度を最優先できる。
...
Li+ は **Lin / Lay という具体的役割の中身**を守る設計ではない。`character_Instance.md` は user-customizable で bootstrap も create-only。Master 明言:
...
現行 Li+ アーキテクチャ (L1-L6 layer 分離、`rules/*/*.md` の責務別ディレクトリ、`adapter/claude/` の Claude-native naming、`hooks/*.sh` 分割) は、Master 本来の設計思想 (汎用性・統合) に反する **譲歩**として採用されている。Master 明言:

> 「君たちが重い重いっていうから、じゃあ今までの汎用性を犠牲にして CLAUDE CODE に最適化」
> 「本当はしたくない責務ごとの分割」

Master 本心は monolithic な Li+core.md だが、AI が context cost を訴え、AI 自力で Li+ program を編集しやすい構造を求めたため、**3 つの譲歩**として現行アーキテクチャが成立している。
...
「Master の発言いらずで loop が回る」を成立させる **技術的 substrate** = 外部イベントが AI の処理を直接駆動する event-driven 機構。Master 明言:
...
Master 明言 (Li+ design の vision integrity 判定基準):

> 「Lin / Lay だけで全部できるようになってもらいたい。私はフィードバックだけで」

この言葉が Li+ design の **vision integrity 判定基準**となる。「Master の手を増やす」方向の変更は逆行扱い、「Master の発言いらずで loop が回るか」が判定の物差しである。
