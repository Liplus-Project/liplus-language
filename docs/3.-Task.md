...
（→ `rules/model/art-of-subtraction.md` の Artifact deletion calibration 節。L1 Model Layer に統合済 (#1351) / #1354 で section 名を再構成）
...
## Research Strategy（governance のみ）

（→ `skills/task-research-strategy/SKILL.md`。L3 Task Layer。トリガー時ロード）

#1220 で機械的 core を `skills/model-agentic-search/SKILL.md` (L1) に encapsulate 済。 本 skill は親 AI が retain する **governance** のみ保持する：

- 検証優先（事実が不確かなら外部検証を優先、速度より正しさを最適化）
- 文脈保全（メイン作業文脈を保全する経路を選ぶ）
- 自発的並列調査（調査対象が issue の場合、判断前にサブエージェントを並列投入。人間の指示を待たない）

source priority 表 / github-rag-mcp の二つの検索面 / 多角度クエリ等の機械的部分は `skills/model-agentic-search/SKILL.md` に移管した。

---

## Retrieval Orchestration（consumption discipline のみ）

（→ `skills/task-retrieval-orchestration/SKILL.md`。L3 Task Layer。トリガー時ロード）

#1220 で機械的 core を `skills/model-agentic-search/SKILL.md` (L1) に encapsulate 済。 本 skill は親 AI が retain する **consumption discipline** のみ保持する：

- budget gate（soft cap 8 / hard stop 12、 task scope inherited）
- 停止判断（partial findings 提出 vs 別ラウンド vs follow-up issue 化）
- naive single-shot defense（結果が「十分」に見えても、機械的 cross-check を skip しない）

### 三 skill の軸分離

| skill | 役割 | 発火 |
|---|---|---|
| `skills/model-agentic-search/SKILL.md` (L1) | 機械的 core: calibration + category OR trigger / multi-angle / 三状態 cross-check / Tier 1-2 escalation | 探索が必要な瞬間 (低 calibration OR 時変キーワード) |
| `skills/task-research-strategy/SKILL.md` (L3) | 親 AI 側 pre-retrieval governance (subagent parallelism / verification-first) | 調査タスク開始時 |
| `skills/task-retrieval-orchestration/SKILL.md` (L3, 本セクション) | 親 AI 側 post-retrieval consumption discipline (budget / stop) | retrieval 結果受領時 |

積層関係にある。 機械的 core が retrieval を実行し、 task layer の二 skill が親 AI 側の規律を保持する。
...
| Deletion Impact | `rules/model/art-of-subtraction.md` (Artifact deletion calibration) | L1 | rules/ 常時 (#1351 で統合 / #1354 で再構成) |
| Research Strategy | `skills/task-research-strategy/SKILL.md` | L3 | skill トリガー |
| Retrieval Orchestration | `skills/task-retrieval-orchestration/SKILL.md` | L3 | skill トリガー |
