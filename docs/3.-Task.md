...
| `review-pending` | 実装フェーズ終了、orchestration（brake 評価 / レビュー / マージ / クローズ）待ち。executor 非依存。subagent は exit のたびに必須、main は best-effort |
...
関係の軸は state 部分集合とは別に置く。作業が *どの issue を* 待っているかはラベルではなく GitHub の Issue Dependencies（`blockedBy` / `blocking`）で表す。state ラベルは作業が止まっていることを、依存は何で止まっているかを表し、別の情報である。`blocked` は人間入力待ちの状態として残し、依存グラフから導いた派生表現にはしない。依存 issue 待ちの `waiting` も状態であり、待っている相手の issue は依存として記録する。誰が記録するかは下記「Issue Format」の「依存」の項（#1687）。
...
- 親 AI 側 post-retrieval 消費規律: 停止判断、naive single-shot defense
...
issue の完了判断は本文項目ではなく、issue 状態と PR/CI/release flow で管理する。本文に専用の完了条件欄を持たない。ただし、PR 単体では満たせない完了条件（deploy 後の本番状態、実機の挙動、cron 数周後の観測、人間の確認など）に限り、issue 本文に完了条件として書く。PR 単体で満たせる条件は書かない。判定は「その完了条件は PR 単体で満たせるか」の二値で行う。
...
issue 本文に欠陥・状態の断定（「◯◯が残っている」「◯◯が起きている」「◯◯が存在しない」）を書くときは、それを見た場所——クエリ、実行結果、literal の `path:line` のいずれか——を併記する。併記できないものは断定形で書かず、仮説として書く。

継承した前提（別 issue・別セッション・過去の自分）を再掲するときも同じ扱いとする。

制御フローの記述は、その記述が依拠する try / catch / return / break の位置を literal 上で確かめた範囲に限る。

issue 本文に誤った断定を見つけたら、close 済みの issue であっても本文を訂正する。

##### 依存

ある issue が別の issue を待っていると分かっているときは、その関係を GitHub の Issue Dependencies（`blockedBy`）として張る。張るのは起票者で、起票時、または依存が判明した時点で、分かっている範囲で張る。親 issue / sub-issue の構造から導出はしない。着手順序は判断としてセッション間・AI 間で再現されるべきものであり、memory には置かない。

書き込みは GraphQL（`addBlockedBy` / `removeBlockedBy`、2つの issue の node id を渡す）で行い、gh CLI の依存フラグ（2.94.0 以降）には依存しない。張った関係は cold-start の dependency ordering surface が読み戻す（`rules/evolution/cold-start-synthesis.md` Dependency Ordering Surface）。張り忘れると順序が嘘になるが、surface は欠けた辺を補修しない ── 起票者が張ることが、この面の正しさの前提である（#1687）。
...
（→ `skills/task-subagent-delegation/SKILL.md`。L3 Task Layer。トリガー時ロード。発火の瞬間で分割しており、委任プロンプト作成は `skills/task-subagent-prompt/SKILL.md`、spawn パラメータ（model / effort 方針 / 並列幅上限）は `skills/task-subagent-spawn/SKILL.md`、subagent 側の state-machine label は `skills/task-subagent-state-labels/SKILL.md`）
...
フォールバックの形：親は著者役のサブエージェントを新規 spawn し、それが issue body・PR diff・ブランチ上の commit・PR コメントスレッド（各ラウンドの所見と、既に済んだ裁定を載せている）から再構成する。役割は変わらないため、上記4点はそのまま注入する。加わるのは1点だけで、それは cold なセッションが自力で供給できないためである——この spawn はフェーズ2から入る。変更は実装済みで PR は開いており、その仕事は裁定であって実装ではない（`skills/task-subagent-delegation/SKILL.md` の2フェーズ）。真の再開はここを述べない。自分のセッションそのものがその陳述だからである。それ以外に差は無い。停止条件も、2つの否定も、指摘の所在も、言語の名指しも同じで、親が指摘を貼らない点も変わらない。
...
サブエージェントは1つの委譲に対して2つのフェーズを実行する。フェーズ1はブランチ作成、実装、コミット、プッシュ、PR 作成、CI ループ。フェーズ2は brake のあるラウンドが所見を投稿した時点で親が再開して始まり、そのラウンドの評価者コメントを PR で読み、各指摘を裁き、受入 / 却下とその理由を同じスレッドへのコメントとして投稿し、受け入れた分を修正、コミット、プッシュ、CI ループする。次のラウンドを開く再開も同じフェーズ2に入り、ラウンドをどこで終えるかは `skills/evolution-parallel-agent-eval/SKILL.md` Procedure の step 8（Round boundary）が持つ。停止条件は各フェーズの末尾と各ラウンドの末尾で到達し、文言はいずれも同一である。
...
分割は非対称であり、前倒しするのは assignee だけである。2つを揃えて親へ寄せれば1箇所で済む、という単純化はこの段落が拒む対象そのものである。`in-progress` は「作業が走っている」という主張であり、実際に走らせている主体しか正しく主張できない。委譲の前に置くと、実装前の調査段階でサブエージェントが落ちた場合にその主張だけが生き残る——誰も走っていないのに「走っている」が残り、他の AI エージェントや別セッションの実行者はその issue を「いま作業中」と読んで触れず、誰も解除しない。false lock である。assignee はその種の主張をしない。「誰に渡ったか」であって「走っている」ではなく、加算的であり、現オーナーは handoff record から読んで assignee 欄単体からは読まない（後述）ため、残留した assignee が次に読む側に生むのは、名指しされた相手か人間への確認1回ぶんの停止であって、誰も解けない lock ではない。この非対称の上に分割が乗っているのであって、整理して片付けられる分担の都合ではない。
...
着手前にこの欄を読む側——他のアカウントが入っているとき着手を止めるか、その停止を何が解くか——は `rules/operations/main-agent-procedures.md` Branch and label flow の Assignee read が正本である（運用レイヤー仕様書「ブランチ作成」）。
...
- APPROVED → PR 作成時点で有効化してある auto-merge の handoff が、この承認でマージを発火させる（`rules/operations/main-agent-procedures.md` の `## Merge Execution`）。実行すべきマージコマンドは残っておらず、したがって委任すべきものも無い。メインエージェントに残るのはマージ後の義務であり、マージを最初に観測したターンで実行する（同節の Post-merge moment）。
