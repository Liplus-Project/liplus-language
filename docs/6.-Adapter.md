...
2. **skills/ ファイル：** リポジトリの `skills/<name>/SKILL.md`（flat 命名、例：`skills/operations-on-commit/`、`skills/agentic-search/`、`skills/evolution-loop/` 等）を `.claude/skills/<name>/SKILL.md` に再帰ミラーする。各 SKILL.md は既に skill frontmatter（name, description, layer）を含んでおり、auto-invocation タイミングを description で宣言する。
