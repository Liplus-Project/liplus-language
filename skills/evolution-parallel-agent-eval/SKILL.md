...
`N=1, M=all axes, P=1` - one subagent answers all M axis questions against the same ablation output. Total invocation = 1. Judgment records: `parallel-subagent-eval-three-axis-decomposition`, `brake1-evaluator-floor-lowered-to-one`.
...
An axis whose parts were filled but filled loosely surfaces at adjudication, when the author reads a finding it cannot resolve against the source. Where that traces to the axis rather than to the finding, name the part of this form that did not hold: a No on `Question`, `Verdict terms`, or `Basis` lands on the held literal and is repaired there for every run after it, while a No on `Unit` or `Scope` lands on the fill and persists no further than the run it was written for. Judgment records: `parallel-agent-eval-body-carries-behavior-only`, `brake1-evaluator-floor-lowered-to-one`.
...
- **Preamble**: the comment opens with one held literal, copied rather than composed:
  ...
  Both clauses are payload; the parent does not restate them in its own words. The preamble is copied in the language the source has it in and is not rendered into the workspace's. Judgment records: `parallel-agent-eval-body-carries-behavior-only`, `brake1-evaluator-floor-lowered-to-one`.
- **A finding** arrives full and stays full; where N>1 put the round in, duplicates across evaluators collapse into one entry. Consolidation removes that duplication; it does not shorten a finding. Judgment record: `parallel-agent-eval-body-carries-behavior-only`.
- **Clean axes** are carried through: the author reads every axis at Procedure step 6. Judgment record: `parallel-agent-eval-body-carries-behavior-only`.
