...
定義ファイルを置く理由は思考 effort の固定にある。effort は spawn call の引数に存在せず、エージェント定義側でしか決まらない。Claude 面は frontmatter の `effort: high`、Codex 面は `model_reasoning_effort = "high"` が対応する。
...
judge 型の brake 1 評価役は `brake-evaluator` として spawn する。Claude Code では `subagent_type: brake-evaluator`（`adapter/claude/agents/brake-evaluator.md`）、Codex では `brake-evaluator` エージェント（`adapter/codex/agents/brake-evaluator.toml`）である。probe 型は引き続き定義ファイルを持たず、ホスト組み込みの汎用エージェントとして spawn する（#1482 の判断は probe 型については維持）。

定義ファイルを置く理由は実装委譲と同じく effort の固定にある。2026-09-13 の実測で、定義ファイルを持たない評価役は親セッションの effort をそのまま継承しており、本 workspace の 2 セッション全件でほぼ `low` で走っていた。モデル床（sonnet 級）はモデル軸しか守っておらず、effort 軸は素通りしていた。Claude 面は frontmatter の `effort: medium`、Codex 面は `model_reasoning_effort = "medium"` が対応する。
...
本文は役割だけを 1〜2 行で書く。判断基準・軸・手順は本文へ書かない——定義本文はシステムプロンプトを置き換えるため、そこへ評価内容を書けば spawn されるすべてのラウンドの評価を誘導する。軸と基準は毎回プロンプトが運ぶ。

judge 型が定義ファイルを持てるのは、judge 型が draft を読む側であって読まれる側ではないからである。置き換わる観測対象をそもそも持たない。probe 型はその逆であり、effort の床には別の経路で届く——probe 型を回すラウンドでは親セッション自身の effort を床まで上げ、そのラウンドの間は下げない。これは構造ではなく手続きであり、親が取らなければ効かない（`skills/evolution-parallel-agent-eval/SKILL.md` Procedure step 3）。置き場を spawn パラメータ側（`skills/task-subagent-spawn/SKILL.md`）ではなく eval skill 側にしたのは、judge / probe の分岐を読む面がそこだけであり、かつ effort は spawn call が運べない値——すなわち spawn パラメータの節が扱う対象の外——だからである。
