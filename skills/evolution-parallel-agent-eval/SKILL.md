...
Every spawn under this skill also explicitly sets the host's background-execution parameter to disabled (Claude Code: `run_in_background: false`; Codex: equivalent synchronous invocation) — omission is prohibited on hosts where the parameter exists and defaults to background. This skill's spawns are frequently nested (spawnDepth >= 2 when brake 1 runs inside an already-delegated implementation subagent), and background-mode nested/grandchild agents have been empirically observed to get stuck showing "running" indefinitely in the host agent panel after they have actually completed (confirmed via transcript `stop_reason: end_turn` plus timestamp inspection against the stale panel state; two occurrences on prior self-evolution PRs). The completion-signal-propagation gap for nested background agents is host-side and out of this repo's control, so the fix here is avoiding the trigger condition rather than patching the host. Foreground/synchronous parallel calls (multiple Agent tool calls in one message, per the Parallel execution sentence above) give the same concurrency without this failure mode, so disabling background execution costs nothing on the concurrency axis.
