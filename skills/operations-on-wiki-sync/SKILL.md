...
- **Index integrity: verify every Decision Structure entry `docs/Decision-Structure.md` names exists on the wiki.** Read the index from the source ref (`git show origin/main:docs/Decision-Structure.md`), strip code notation from it with the algorithm below, and build the expected slug set from its wiki link targets, the same extraction step 4 uses to build `wiki_only`. STOP if any expected slug has no `{tmpdir}/<slug>.md`.
  ...
  The opposite direction, a wiki entry the index does not name, is not asserted here: step 4 builds `wiki_only` from this same index, so such an entry is already in that step's `unclassified` set and the run has stopped before this assertion is reached. Do not add a second detector for it. What step 4 carries instead is the repair direction, stated there.

  Neither direction needs a filename rule to tell a Decision Structure entry from a requirements or reference doc. Here the expected set is the index's own targets; at step 4 ownership is decided by location (a top-level `docs/<name>` counterpart on the source ref) plus the wiki-only list, which is what puts `1.-Model.md`, `A.-Concept.md`, `Home.md`, `_Footer.md`, `DiDD.md` and `Decision-Structure.md` on the docs/-owned side without naming any of them. A pattern over filenames — a leading digit, a leading letter-dot — would be a third ownership system standing beside those two, and the two already decide every page.
     ...
     if [ ${#unclassified[@]} -ne 0 ]; then
       for name in "${unclassified[@]}"; do
         if [ -n "$(git log --diff-filter=AD --format=%H -1 "$SRC_REF" -- "docs/$name")" ]; then
           ...
           echo "unclassified: $name (no docs/ history; likely wiki-source — for a Decision Structure entry the repair is its docs/Decision-Structure.md row, not a deletion)"
         fi
       done
       exit 1
     fi
