...
Two consequences are load-bearing here. First, the round trips carry no cap: what ends the loop is the skill's Procedure step 8, whose recurrence stop hands the loop to the human, and convergence does not rest on `skills/model-loop-safety` held by an actor inside the loop. Second, resumption keeps the implementation context, so the "parent reconstructs context from the report" cost that `skills/task-subagent-delegation/SKILL.md` Rules accepted is not paid; the always-delegate rule keeps its own axis (simplicity) unchanged and loses a branch. What ends the loop, what one round trip is, and what standing a rejection has inside the loop are the skill's; none of them is restated here, for the reason the brake 1 paragraph below states about copies on this surface.
