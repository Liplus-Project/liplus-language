...
親 issue = 1ブランチ。`gh issue develop` は親 issue のみに対して実行する。sub-issue（子・孫・…）は親ブランチ上でコミットする。sub-issue ごとの個別ブランチは作成しない。PR マージで親 issue が自動クローズされるが、これは意図通りである（全 sub-issue が同一ブランチで完了してからマージするため）。sub-issue に独立ブランチが必要な場合は、別の親 issue を作成する。
