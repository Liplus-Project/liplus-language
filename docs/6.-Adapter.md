...
`skills/task-subagent-delegation/SKILL.md` に基づく委譲は、ホスト組み込みの汎用エージェントではなく Li+ が配る実装委譲先を spawn する。Claude Code では `subagent_type: implementer`（`adapter/claude/agents/implementer.md`）、Codex では `implementer` エージェント（`adapter/codex/agents/implementer.toml`）である。

Claude 面で定義ファイルを置く理由は、役割に加えて思考 effort を固定することにある。Claude の spawn call は effort を運べないため、frontmatter の `effort: high` がその値を置く。Codex 面の定義ファイルは役割本文と sandbox を運び、effort は spawn call の `reasoning_effort="high"` が置く。`adapter/codex/agents/*.toml` に `model_reasoning_effort` は書かない——定義ファイル側の値は per-launch で解決した値を上書きするためである。
...
本文は単体で成立する形で書く。ホストが基底プロンプトの上に重ねるのか置き換えるのかは判別が付いていない（2026-09-11 の実測で判別不成立）。未測定の基底に対する差分として読ませる本文は、その未決着がそのまま壊す形になる。

brake 1 の評価者は下の節が扱う。Claude の probe 型は素の挙動そのものが eval の読み取り対象であるため定義ファイルを持たない。Codex は judge / probe の両方で定義ファイルを選ばず、役割と effort をそれぞれ self-contained prompt と per-call 引数から渡す。

定義ファイルの更新は、走行中のセッションに対して次の spawn から効く。再起動は不要だが通知も出ないため、委譲が走っている最中に変更を落とす場合は人手で周知する。

### brake 1 評価役のエージェント定義

Claude Code の judge 型は `subagent_type: brake-evaluator`（`adapter/claude/agents/brake-evaluator.md`）として spawn し、probe 型はホスト組み込みの汎用エージェントとして spawn する。Codex は judge / probe の両方を定義ファイルなしで spawn し、judge の役割は self-contained prompt が運ぶ。`adapter/codex/agents/brake-evaluator.toml` は #1968 で追加された役割本文と sandbox を保持するが、Codex の brake 1 spawn は選択しない。

2026-09-13 の Claude 実測では、定義ファイルを持たない評価役が親セッションの effort を継承し、本 workspace の 2 セッション全件でほぼ `low` で走っていた。モデル床（sonnet 級）はモデル軸しか守っておらず、effort 軸は素通りしていた。Claude judge は frontmatter の `effort: medium` で塞ぐ。Codex は spawn call が `reasoning_effort` を運べるため、judge / probe とも `reasoning_effort="medium"` を明示し、定義ファイルの優先順位に奪わせない。
...
`model` の sonnet 級床は両ホストとも spawn call で明示する（`skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Model floor）。Codex の `model` と `reasoning_effort` は独立した per-call 軸である。`tools` も定義ファイルへ書かない。評価者のツール権限を絞らない判断と、read-only を `tools:` で表現しない判断は同 skill の Non-scope が持つ。Codex 面の保持された `sandbox_mode` を `read-only` にしないのも同じ理由であり、加えてリポジトリ全体を掃く軸が自分の作業ディレクトリへクローンする許容がある。

保持する定義本文は役割だけを 1〜2 行で書く。判断基準・軸・手順は本文へ書かない——定義本文はシステムプロンプトを置き換えるため、そこへ評価内容を書けば選択時のすべての評価を誘導する。軸と基準は毎回プロンプトが運ぶ。

Claude judge が定義ファイルを持てるのは、draft を読む側であって読まれる側ではないからである。Claude probe はその逆であり、親セッション自身の effort を床まで上げてラウンド中は保つ。Codex では per-call の `reasoning_effort` が両型へ届くため、親セッションの値を動かさず、両型とも定義ファイルを選ばない。値の選択は judge / probe の分岐を読む `skills/evolution-parallel-agent-eval/SKILL.md` が持ち、省略禁止とモデル別の許容値確認は `skills/task-subagent-spawn/SKILL.md` が持つ。
...
- これらの拘束は spawn call ごとに行う。`adapter/codex/agents/*.toml` に `model_reasoning_effort` を固定しない——定義ファイル側の値が per-launch の解決値を上書きするためである。
