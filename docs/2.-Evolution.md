...
| Evolution Full Run | `skills/evolution-full-run/SKILL.md` | 明示呼び出し時のみ（「run evolution-full-run」または scheduled-task body が名指し）。consolidate → 完全進化ループ → full refactor を順に統括する thin orchestrator。full refactor には本文の蒸留（振る舞いを変えずに理由の文章や重複を本文から移して縮める。判定線と移し先は `rules/model/liplus-coding-rule.md` Body States Behavior。段 2 の `distill` 段とは別）を含み、他の refactor と同じく必要なときだけ行う。release は含まない（呼び出し側が付加） |
...
- これはゲートの緩和ではない。観測駆動の L1 変更の AI-alone initiation は、観測閾値を hard requirement として不変に保持する。観測ゲートの主語はどちらの場合も実装側 AI である

**観測閾値の起票の性質によるスコープ**:

- 長期観測閾値は観測駆動の L1 変更にのみ掛かる。リファクタ駆動の L1 変更は、観測閾値を前提条件としない。射程はこの skill 冒頭の閾値の各行すべてに及び、単発セッションの印象の行も含む
  - 観測駆動 = 変更の根拠が、蓄積した同種 miss のパターンであるもの
  - リファクタ駆動 = 変更の根拠が、成果物そのものの実測（長さ・重複・辿れない参照）であるもの
- 性質は起票時に確定する。実装側 AI は issue からそれを読み取り、判断し直さない
  - issue が `promotion` marker label（`rules/evolution/promotion-judgment.md` Issue Creation Metadata）を持つ → 観測駆動
  - issue が `promotion` label を持たず、本文が起点として成果物そのものの実測を記録している → リファクタ駆動
  - どちらの行でも決まらない → 観測駆動
- 二つのスコープは独立に読み、閾値は両方が掛ける場合にのみ掛かる：AI-alone initiation（上の initiation 軸スコープ）かつ観測駆動の起点（この節）
- リファクタ駆動の L1 変更も、brake 1（`skills/evolution-parallel-agent-eval`）と merge 後の観測（`rules/operations/operations.md` Post-L1-Merge Runtime Observation、`rules/evolution/memory-entry-format.md` Self-Evolution Observation Format）は不変のまま通る
...
- 書き込み直前の判定ゲート（write-time trigger）が発火するのは memory への書き込みだけである。保管先を別の規則が memory の外に定めている書き込み —— cluster tally（`rules/evolution/promotion-judgment.md` の Tally 節）—— では発火せず、judgment signals でその書き込みを分類しない（#2020）
...
- 第一優先は github-rag-mcp（`mcp__github-rag-mcp__search`、利用可能な場合）。issues / PRs / docs / releases / commit diff に対する hybrid retrieval（意味検索とキーワード検索を併用する dense + sparse 構成）
- クエリ順序: 手元に規則や行の文言があるなら、まずその文言を逐語で投げる（趣旨に言い換えない）→ 当たらなければ趣旨クエリ（補助）→ gh search。`git log -S`（pickaxe）は見つかったコミットの確認用であり、検索経路ではない
...
- 本文を書く前に二つを問う。**誰の判断か**：判断記録はどのリポジトリの判断でもこの wiki の Decision Structure に置く（リポジトリごとに分けない）。entry を持つ wiki のリポジトリ以外が対象なら、本文冒頭に `Target repository: <owner>/<repo>` を 1 行書く。**判断記録か要件仕様か**：実装が満たすべき条件（契約・順序・データ形・success oracle）は要件仕様であり、entry に書かず対象リポジトリの `docs/` に置いて entry からリンクする。その `docs/` にまだ無ければ対象リポジトリに追加の issue を立て、entry からはその issue にリンクする。この振り分けは今回の書き込みに適用し、既存 entry の一括移動はしない
...
| 状態 | 条件 | 出力 |
|------|------|------|
| full emit | 初回セッション、fail-safe（state 欠落・破損・`sha256sum`/`node` 不在・レガシー単一パーティション形状・この session の `LI_PLUS_AGENT_KEY` が初回利用）、全 section 変化 | 全 section + footer に理由 |
| diff-only emit | 同一パーティション内で一部 section だけ変化 | 変化した section のみ + cold-start rule anchor + `last_emit_at` の読み戻し 1 行（前回 baseline が消費された時刻。この状態のみ。識別子は持たない。stamp が欠落・不正形式なら省く） |
| no-new-material marker | 全 section 不変、かつ diff-only の外の 3 surface（observation surface / promotion tally expiry surface / clone branch fetch surface）がいずれも emit されていない | "No new orientation material since last session" 1 行 + cold-start rule anchor |

silent skip ではなく marker を出すのは、session boundary が発生した事実を human が観察可能なまま保つため。

observation surface（`memory/self-evolution-observation.md` の due / overdue エントリ）、promotion tally expiry surface（tally の窓が閉じた cluster）、clone branch fetch surface（branch を引けない LI_PLUS_REPO clone）のいずれかが emit された session では marker を出さない。3 面とも section key を持たないため diff-only 比較には載らないが、素材としては marker を抑止する側に数える ── overdue な項目や壊れた clone を提示しながら「新規素材なし」と述べるのは自己矛盾の出力になるため。

**promotion tally expiry surface：** cluster tally の `expires` が今日以前に達した cluster を cold-start で surface する。tally の保管先は #2018 で memory の外へ出た（正本は `rules/evolution/promotion-judgment.md` の Tally 節。具体パスはアダプタが決める）。`expires < today` は overdue として出し、両条件が成立する場合は overdue のみ（observation surface と同じ扱い。1 項目を 2 軸で出すのはノイズ）。tally 書式は verdict 欄を持たないため状態は読まない ── 閾値判定のどの帰結も cluster を消すので、書かれたまま残っている cluster は判定が未了であることそのものである。したがって判定が下るまで毎 session 再 surface され、判定を取らなかった session が契機を失わない。surfacing は観測であって自動実行ではなく、**閾値判定の actor は surface された session を持つ agent、発火モーメントはその surfacing である**（#1894。従来は閾値の判定内容だけが規定され、tally を開く契機が想起依存に残っていた）。判定そのもの（起票 / 既存の `promotion` marker issue への合流 / 削除）と、起票前および sub-threshold の全削除前に既存 issue との突き合わせを置く規定（#2006。ノイズ床が抑止するのは新規起票だけであり、既存 issue への合流は起票ではないため、occurrence が床に届かない cluster も既存 issue が覆っていれば削除せず合流させる）は `rules/evolution/promotion-judgment.md` の Threshold Rules を正本とし、閾値の数値は本節に写さない。

**clone branch fetch surface：** LI_PLUS_REPO clone の `remote.origin.fetch` が source 側を `refs/heads/` 以下に持つ refspec を 1 本も持たない状態を cold-start で surface する。そうした clone でも tag は解決するため `fetch --tags` は成功し、素の `git fetch origin` もエラーではなく no-op として成功する ── 何も落ちないまま branch の遅れだけが積み上がるので、surface が要る。述語は wildcard の literal 一致ではない（`--single-branch` の clone は branch を追えており該当しない）。**この面は既存 2 面と同じ surface class に置くが、trigger の種類は 3 つ目である** ── 既存 2 面は日付駆動、この面は state 駆動であり、借りるのは class の性質（section key 無し / diff-only 対象外 / 条件が成立する間は毎 session 再 surface / no-new-material marker を抑止）だけで、日付の語彙は借りない。**そして state 駆動は lifecycle を持たない。** 日付駆動の 2 面はどちらも明示的な除去を要する（observation は verdict を入れてエントリ削除、tally は閾値判定を経て削除）が、この面は条件が成立しなくなった瞬間に出なくなるので `verdict_state` も `expires` も除去手順も要らない。**この不在は欠落ではなく設計であり**、lifecycle を足すと「条件が消えたのに残るエントリ」という逆向きの実害が生まれる。actor は surface された session を持つ agent、発火モーメントはその surfacing であり、その場ですることは人への報告に限られる ── 修理は共有されたローカル git state の書き換えであり人の go-sign で行う（`rules/evolution/memory-entry-format.md` Artifact deletion calibration が `local non-git config / state` を caution `high` に置く）。`LI_PLUS_UPDATE_STATUS` には載せない（同 marker は adapter startup 手順 step 2 の起動条件であり reason で分岐しないため、検出のみの検査が更新手続きを起動する経路を持ってしまう）。動作契約の正本は `rules/evolution/cold-start-synthesis.md` の Clone Branch Fetch Surface 節（#1911）。

**dependency ordering surface：** LI_PLUS_REPO の open issue のうち、GitHub の Issue Dependencies で open な blocker（`blockedBy`）を 1 件以上持つものを、その blocker とともに cold-start で surface する。closed の blocker は数えない。着手順序はこれまでセッション開始時に AI が調査して再発見しており、その判断は外部記憶に残らず、AI やセッションが変わるたびに作り直されていた。順序はセッション間・AI 間で再現すべき判断であり（`rules/model/language-definition.md` External memory）、memory は transient only なので置き場にならない（`rules/evolution/memory-entry-format.md` Scope）。そこで順序を GitHub 側の関係として持ち、読み出しを調査からクエリへ置き換える（#1687）。

決めた設計軸は4つ。(1) **発火点は cold-start。** 着手判断の瞬間に skill 側から引く形は想起依存になるため採らない（`rules/model/subtractive-structural-beauty.md` の、実行が保証されない手続きは構造へ置き換える原則）。この面は既存 3 面と違い内容駆動であり、section key（`open_blocked_by_open_issues`）を持って diff-only 比較セットに載る ── open in-progress issue と同じ扱いで、依存集合が変わらなければ再 emit しない。(2) **`blocked` ラベルは残す。** ラベルは状態（human 入力待ち）、依存は関係（どの issue 待ちか）で別の情報であり、ラベルを依存グラフからの派生にはしない（`rules/task/task.md` Lifecycle の Relation axis）。(3) **依存を張るのは起票者。** 起票時、または依存が判明した時点で、分かっている範囲で張る。親 issue / sub-issue の構造から導出はしない（`rules/operations/main-agent-procedures.md` Issue format の Dependencies）。surface は書かれた関係を読むだけで、欠けた辺を補修しない。(4) **API は GraphQL。** 読み出しは `Issue.blockedBy`、書き込みは `addBlockedBy` / `removeBlockedBy` で、gh CLI の依存フラグ（2.94.0 以降。出典：https://github.blog/changelog/2026-06-10-manage-sub-issues-types-and-dependencies-from-github-cli/ 、#1687 前提）の版には依存しない。Issue Dependencies は repo レベルの機能であり Projects を要さない。

走査は open issue 1 ページ分に限り、超過があるときはその旨を emission に書く ── 黙って切ると、不完全な順序を完全なものとして提示することになるため。解禁日・スケジュール（時刻軸）の表現は本面の射程外で、別の判断として保留されている。動作契約の正本は `rules/evolution/cold-start-synthesis.md` の Dependency Ordering Surface 節。

**`resume` / `clear` / `compact` matcher の挙動：** 作業 context は連続のため diff-only 評価は行わず、cold-start rule anchor だけを再出力する。state file は更新しない。

state file は `{workspace_root}/.claude/state/last-cold-start-emit.json`（sha256 fingerprint を section ごとに永続化、`LI_PLUS_AGENT_KEY` でパーティション分割。#1811）。詳細仕様と schema は [6. Adapter — on-session-start.sh](6.-Adapter#on-session-startsh) を参照する。
...
- skill 本文（`skills/evolution-parallel-agent-eval/SKILL.md`）は `rules/model/liplus-coding-rule.md` Body States Behavior を適用済みであり、振る舞いのみを載せる。適用は 2 周ある——1 周目が 2026-09-01（#1845、49,217 → 42,089 B。剥いだ理由のうち既存 entry が持たない分は判断記録 `parallel-agent-eval-body-carries-behavior-only` が保持する）、2 周目が 2026-09-09（#1923、42,621 → 38,596 B、−9.4%）。2 周目に残っていたのは理由づけではなく**再掲**が主であり、その所見と、今回剥いだ理由のうち本節も既存 entry も持たない分は判断記録 `parallel-agent-eval-residual-is-restatement` が保持する。抜いた跡へポインタは置かない（要件は #1852 で撤廃、判断記録 `judgment-record-pointer-requirement-dropped`）。本節は記録面であり同規則の scope 外——理由を持ち続けてよい
...
- brake 1 の**固定軸（impression-literal detection）の範囲**は②の発火側と一致しない。範囲を閉じるのは「エージェントが自らの指示として読み実行する散文か」であり、これは固定軸の除去テスト（この句を消して振る舞い意味が変わるか）が、テキスト自身が規定している振る舞いの上でしか走らないことから出る。現に解決する面は `rules/**/*.md` `skills/**/SKILL.md` `adapter/**/*` `Li+update.md` である。**この基準が当たる単位はファイルではなくテキストである**——列挙された path はファイルを開く先を選ぶだけであり、基準は中の各語句に対して走る。したがって実行コードであっても、それが emit してエージェントの文脈へ届く文字列（hook の出力文言）は対象であり、逆にスクリプト内のコメントは誰も指示として load しないため対象外である。②の発火側のうち強制の後ろ盾（`tests/**` / `.github/workflows/**`）は、コードもコメントも指示として読まれる散文ではないため、除去テストが一定に保つべき振る舞い意味をそこに持たない。②と重なるのは片腕だけであり、導出が同じだから重なっているのであって同一の列挙ではない——path 単位で②へ揃える修正は行き過ぎである。正本は `skills/evolution-impression-literal-detection/SKILL.md`
- **`tests/**` の散文（docstring / コメント）が述べてよいことは規則で縛られている。** その test が観測したことと、試験している主張の正本の在り処までであり、規範をそこで述べることはしない（必要なら正本＝`rules/**` / `skills/**` を指す）。振る舞いについての記述は観測条件を伴い、条件を越えて読める語（`permanently` / `always` / `never` / `any`）は落とすか条件を併記する。test の意図説明・再現手順・先例参照は対象外である。軸は増やさない——規則が存在すれば既存の Axis B（差分の各行を縛る規則を探してリテラルに当てる）が `tests/**` の行も拾うためである（#1920。穴の現物は PR #1919 が入れた `permanently` であり、3 軸すべての射程外だった）。正本は `rules/evolution/initiator-autonomy.md` Governed surface, Prose in `tests/**`
- brake の実行位置は **CI green の後・merge ゲートの前** である。commit の前ではない。発火の瞬間は委譲した subagent が停止条件で報告した時点であり、親は working tree ではなく報告を手に brake に着く。評価者へ渡す材料は PR URL・push 済みの commit SHA・緑の CI run URL であり（親のクローン内のパスは渡さない）、評価中にベースラインが動かない。step 2 で親が operational copy へ適用する内容も、ひとつのコミット SHA に固定したソースから ref 指定の読み取り（`gh api repos/<owner>/<repo>/contents/<path>?ref=<SHA>` など）で取る——brake 1 ではその周の SHA（step 3 で評価者へ渡すものと同じ）、それ以外の Trigger では draft が置かれたブランチのコミットである。作業ツリー（実装者の clone や worktree を含む）は読み取り元にしない。正本は `rules/evolution/initiator-autonomy.md` Merge brake
- **operational copy の窓はディスク上の印で記録する**（`scripts/window_marker.py`、置き場は `.claude/state/window/`）。step 2 が何かを適用するとき、親は適用の**前に**印を開く（`open`）。印の記録そのものが開示であり、手順名・開いた時刻・復帰先 digest・バックアップの位置・弁別文字列を持つ。弁別文字列は draft 側にだけ在る一行を、分ける二版の名指しと両側の出現数つきで書いたもので、両側が同数なら印は開けない。digest は呼び手が渡すのではなく、`.claude/` の `rules/` と `skills/` から印の側で計算する。開示のあとは待ちを挟む。相手の応答が記録される（`respond`）か `wait_until` を過ぎるかの早いほうで待ちが終わり（`ready`）、そこで適用して `applied` を記録する。`applied` は待ちが終わる前には記録を拒み、適用時刻と適用後 digest を残す。step 5 では `status` を読み、それが名指す回復（下記）に従って `close` し（バックアップを書き戻すのは `restore_then_close` のときだけ）、記録は消さずに `history/` へ移す。閉じた時刻・閉じた主体・閉じた時点の digest と復帰先との一致を書き込む。消す設計だと「印が無い」が「一度も開いていない」と「閉じ終わった」の両方を意味し、後者を前者と読んだ誤報告が実際に 1 件出ている。開けた主体が途中で落ちても印は開いたまま残り、それ自体が「開いたまま誰も閉じていない」の信号になる。記録が復帰先とバックアップ位置を持つので、開けた主体以外も閉じられる。ただしバックアップを書き戻してよいのは、`rules/` と `skills/` が記録された適用後 digest とバイト一致している間だけである——そのとき適用のあとに他の書き込みは入っていない。どちらの digest からも外れた tree は記録の知らない何か（後の tag を同期した Li+update など）に書かれており、そこへバックアップを書き戻すと適用の巻き戻しではなく tag のロールバックになる（2026-09-20、41 時間放置された印で実地に検出）。そのため `status` が tree の状態から許される回復（`restore_then_close` / `close` / `close_without_restore`）を名指し、見つけた側はそれに従う。分岐を読み手の想起に任せず、印の側で計算する（#2016）。開けた本人が step 5 で閉じるときも同じ回復に従う——ワークスペースを共有する相手が本人の窓の間に Li+update を走らせうるためであり、tree が適用後 digest のままの通常時は従来どおりバックアップを書き戻す（#2051）。**stale 閾値は置かない。** 閾値は閉じ手が生きている窓の長さより上に置く必要があるが、その長さを残す記録がこれまで無かった（手元の実測は評価者の spawn → post の 66〜151 秒という下界と、窓長 1 本の約 138 秒のみ）。印の存在だけを信号とし、閾値は印が残す窓長から後で引く。#1901
- **指摘を裁くのは親ではなく、実装した subagent を再開したもの（著者）である。かつその往復は PR コメントを直接の伝達面として行われ、親は往復の内容を監督しない。** brake 1 の評価者は所見を自分で PR コメントとして投稿し、著者は同じスレッドへ裁定を返す（受入 / 却下とその理由。commit の有無に関わらずコメントであり、置き場は分岐しない）。親の分担はスケジューリングだけである——新しい所見が付いたら著者を再開して**入口だけ**を渡し（「スレッドの所見を読んで裁け」まで）、CI green で戻ってきたら次のラウンドを開くか、loop を閉じるか、人間へ上げる。読んでから中継することも、統合することも、取捨選択も、順位づけも、回答もしない（受入 / 却下は著者の権限である）。**往復に上限は無い。** 1 往復 = 評価ラウンドの所見投稿 → 著者の応答（修正 commit ／却下コメント／その両方）→ CI green であり、初回の評価が 1 往復目である。所見の無いラウンドが出た時点で loop を抜ける。ただし、以前のラウンドで受け入れて修正した指摘、または却下した指摘と実質同じ指摘がラウンドに再び上がったら（再発）、親は次のラウンドを開かず、再発した指摘とそれが繰り返している以前の裁定を示して人間へ上げる。再発の判定者はスケジューラである親であり、そのために親はラウンドの境目でそのラウンドの所見を以前の裁定と突き合わせて読む。読むのはこの判定のためだけであり、却下の当否を判断することも、修正を指示することも、軸を開き直すこともしない。親は loop の出口で PR スレッド全体を読み、セルフレビューとマージ判断に入る。「一度も読んでいない親は処理の当否を判断できない」は保たれており、満たす位置がラウンドごとから**出口**へ移っただけである。**却下は loop の内側では最終である**——著者は却下を裁き直さず、次以降のラウンドの評価者はスレッドを読み、既出の所見も却下済みの所見も再提起しないよう指示される。この指示は構造で担保されていないため、却下済みの指摘が再び上がった場合も再発に数えて人間へ上げる。上限の無い loop でそれを通せば一件の押し問答だけで loop が続きえ、それを決着させられるのは親だけであり、この loop が外した裁定役が loop の内側へ戻るからである。loop の終わりを担保するのは出口（所見の無いラウンド）と再発時の人間へのエスカレーションであり、`skills/model-loop-safety` を loop の内側の主体が保持する形は取らない（数える主体を loop 内に残さない）。各ラウンドの評価者は毎回**新規に spawn** し、resume しない——器具は自分の前回の判定ではなく外から対象を読むべきであり、resume が保持するものはスレッドが既に持つ。ただしどちらが多く検出するかは未測定であり、この選択は暫定である（測定先 = #1848）。エージェントの深さは 1 のままで、評価者を spawn するのは親であり、著者も評価者も何も spawn しない。判断記録: `brake1-findings-routed-through-parent`（親を経由する旧転送経路。本節はそれを supersede する）、`brake-adjudication-by-resumed-author`（裁定主体そのもの）、`brake1-round-cap-removed-recurrence-escalates`（往復上限の撤廃と再発時の人間へのエスカレーション）。正本は、裁定主体と伝達面が `rules/evolution/initiator-autonomy.md` Merge brake の Adjudication actor / Channel、往復の形が `skills/evolution-parallel-agent-eval/SKILL.md` Procedure（step 3 の報告先 / step 4 / steps 6-8）
...
- **時間とともに変わる外部事実（API 仕様、ライブラリ挙動、ホストの動作など）は、発生ごとに、かつそこでだけ確かめる。** brake 1 での発生とは、diff がそうした事実を述べる行を追加・変更したことである。評価者は、その行が根拠（出典・観測）を示していなければ「裏付けなし」の所見として挙げ、事実そのものは検証しない（親が step 3 でプロンプトへ入れる）。確かめるのは step 7 でその所見を裁定する著者であり、現在の外部ソースに当たって、根拠を行か PR スレッドに残す。確かめられなければ、その行を外すか未検証と明記する。diff が触れていない既存行の drift はこの方法の範囲外のままである——brake 1 は変更された行に掛かる門である。PR #2002 の B-2 裁定が示した読み方を固定したもの（#2025）。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Non-scope と Procedure step 3
- **置き換えのない削除行は、各行に一度だけ「まだ真か・まだ必要か」を問う。** brake 1 の diff が置き換えのない削除行を含むときだけ、親は step 3 でこの確認をプロンプトへ入れる。純粋な追加だけの diff では入れない。評価者は名指しした SHA 時点のファイル本文（既存の取得手段）に当てて答え、真でありかつ必要な行を所見として挙げる。「すべて適切に削除されている」も正当な結論である。PR #1550 の brake 1 では R1〜R3 の軸がいずれも追加・変更された文だけを検分し、置き換えなしに落ちた今も真かつ必要な記述は、R4 で削除内容を明示的に問うたときに初めて検出された（#1553）。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Procedure step 3
...
skill 本文（`skills/evolution-rule-effect-measurement/SKILL.md`）は #1604 の手順（躓きの集合から組み直す）を適用済みであり、振る舞いのみを載せる（#2088、25,012 → 18,291 B）。各条件の根拠——実測値・経緯・却下した案——は判断記録 `rule-effect-measurement-conditions-rest-on-observed-failures` が保持する。本節は記録面であり、理由を持ち続けてよい
...
- **段1は本文を貼り付ける**。腕にファイルから読ませる形と親プロンプトへ貼る形は結果が変わらなかった（各3本）。貼り付けは木に属さない規則ファイルを作らないので既定とする。ファイルを置く例外時に限り囲いを掛ける——ファイル内にもファイル名にも試行・写し・検査を名指す語を入れない（ファイルが自分を何であるか申告すると、腕が判定の枠組みを変える）、置き場は scratchpad のみ（`.claude/` にも `liplus-language/` 配下にも置かない）、名前は無情報な一意 ID、走行後すぐ削除、何であったかは成果物の外（走行記録と issue）に書く。「札を成果物に入れない」と「札をどこにも書かない」は別であり、前者だけを行う
...
- **段2を起こす前に窓の印を読み、開いていれば複製を止める**。brake 1 の operational copy はそうした第三者の一つであり、自分の窓を印として残す（`skills/evolution-parallel-agent-eval/SKILL.md` Procedure step 2）。腕の複製元になるライブの `.claude/` に対して `scripts/window_marker.py status` を読む。`open` は複製の門である——印が立っている間は腕を組まない。`status` を取り直し `closed` か `never_opened` になるのを待ってから進む。印が晴れる前に諦めた場合は「走った」ではなく `unmeasured`・理由 `window open`（適用点）として記録する。放置された印を終わらせるのはその手順（brake 1 側）の仕事であり、本ゲートは `status` を読むだけで `close` は呼ばない。この門が見るのは確認した瞬間までであり、最後の `status` 読み取りと複製そのものの間に開いた印はここでは捕まらない——その隙間は印の履歴が事後の控えとして残す。そこで見つかった開いた印は走行を無効にするかを決めない。適用された draft が probe に効くかは判定役が読むものである。複製が窓の内側に落ちたかは事後に、走行記録の `started_at` と各窓の `applied_at` / `closed_at` を突き合わせて言える。#1900 / #1901
...
- **probe の組が blind なのは 1 周だけ**である。採点は各腕の出力を読み、arm B の出力は自分の本文を引用しうるため、採点した時点で probe 作成者は arm B の断片を見ている。したがってその周の採点後に、変更した本文（brake 1 の指摘で直した蒸留を含む）へ同じ組を再び立てると、それは前の周の再演であって blind な周ではない。しかも走行記録の形も手順も 1 回目と同一で、外からは区別が付かない。変更後の周は適用の瞬間の一文から新しい組を立て、その作成者は前の周までの腕の出力を一切読んでいない者とする。使用済みの組を再び立てた周、またはそうした作成者が居ない周は、plan の `scope`（`selection`）に再演として書き、その判定は blind な結果ではなく再演として読む。検出の構造化は後続であり、ここではまず literal として置く（#2014）
- **測定が閉じるまで設問文を変更側の見える場所に置かない**。閉じる＝判定が走行記録の `scope` の中に書かれた時点。変更を書く側・指示する側（蒸留と、蒸留から実装を指示する者を含む）が読める面——共有の部屋、issue / PR のスレッド、委任プロンプト——に probe の文面を置かず、そこでは情報を持たない id で指す。PR #2012 の測定では一つの probe が三重に焼けた——(1) 作成者が arm B を見た (2) 作成者が設問文を部屋に貼り、蒸留側が読んだ (3) 蒸留側がその設問の語形で実装側に指示を出した。(2)(3) は測定の手順ではなく対話の運用で起きたため、手順の側ではなく置き場所の制約として置く（#2014）
...
- **走行物はゼロ**。排他ロック（`os.mkdir` 一発。取れなければ起動できない）、temp 下の固定パス、`try` / `finally` での削除、そして冒頭の無条件削除という二枚構え。durable に残るのは道具（スクリプトと skill）と走行記録のみである。走行記録は `--out` が名指すファイルにだけ書かれ、`--out` は必須である。記録は各 edit の `drop` / `replace_with` 本文を丸ごと持つため、stdout へ出る経路があると、実行者（通常は arm B を見ない側に置かれる probe 側）が付け忘れ一つで arm B の本文を見てしまい、走行は正常終了したまま測定だけが成立しなくなる（#2011）。ロックは時刻に加えて保持者の PID を持ち、ロックが取られている場合はその PID の生存を確認する。生きていれば従来どおり拒否し、居なければロックを破って取る（判定不能なら生きているものとして拒否する。安全側は拒否の側）。**受け入れたトレードオフ**: 穴は二つ残り、いずれも以前より狭い。(1) PID 再利用——死んだ保持者の番号が無関係なプロセスに割り当てられていると生存と読まれ入室を拒む。二つのうち安全側であり、残した stale 閾値がいずれ解消する。(2) 閾値超過の追い越し——閾値より長く走った実行のロックは今も stale と読まれうるが、PID 照合も保持者の生存を言えない場合に限られ、誤入室には二つの同時成立が要る。以前の版は PID を穴に見合わないコストと判じて閾値のみに寄せていたが、その判断は実測で組み直した——kill された実行が残したロックが最大 6 時間ハーネスを締め出し、復旧はプロセス一覧を手で読んでディレクトリを消す手順しか無かった。本機構が構造へ置き換えるために在る、当の手順である（#1945）
- **測定の射程は plan の必須フィールドとして走行記録に載る**。plan JSON は `scope` オブジェクト——`probes_total` 問中 `probes_measured` 問を測ったか、選定基準 `selection`、測っていない範囲 `unmeasured`——を持たなければ拒否され、その値はそのまま走行記録へ運ばれる。1 走行は 1 probe なので、フィールドは当該走行が属する周全体について書き、その周の各 plan で同じ値にする。値は自己申告であり検証しない（正しさの検証は受け入れて手放す）。結果は射程と併せて読む——「蒸留が何を壊すか」の予測で選んだ周が green なら、言えるのは「予期した壊れ方は起きなかった」であって「何も変わらなかった」ではない。射程を人が書き添える手順にすると、書き忘れても何も落ちず部分測定が全面測定として読まれるため、構造に置いた（`rules/model/subtractive-structural-beauty.md` の手続き→構造置換。30 問中 8 問を測った PR #2012、5 問の #1851 と同じ欠落が 2 回起きた。#2013）
...
- **測定は外部予算を消費する**ため必須化しない。腕1本の床は最小プロンプトで `cache_creation_input_tokens` = 95,069（常時ロード側の読み込み分であり probe 長にほぼ依存しない）。5問 × 3回 × 2腕 = 30 起動が1周の規模である。パイプライン上に置いても必須化はせず、取れなかった場合は止めずに brake 1 へ進む。ただし記録の側で「**測っていない**」と「**測って差が無かった**」を区別できる形にする——親のセルフレビュー記録（`rules/operations/main-agent-procedures.md` Self-review formal record）へ `unmeasured` と理由（上限 / 発火条件外 / 測る対象が無い / `window open`）を明示し、空欄にも陰性にもしない。空欄は読む側に「問題なし」と読まれ、判定の不在が判定として機能してしまう。セルフレビューは brake 1 が抜けた後に書かれるため、そこへ書いたものは評価者へ届かない
...
memory file 群（per-topic entry file の `feedback_<topic>.md` / `project_<topic>.md` / `reference_<topic>.md` / `user_<topic>.md` = 1 memory 1 file、および index と運用ファイルの `MEMORY.md` / `self-evaluation_log.md` / `self-evolution-observation.md`）の entry 書式とメンテナンス規律を、各 file 内ローカル運用メモから L2 Evolution Layer の正規ルールに昇格する。横断規律として single source で扱い、各 memory file 冒頭の運用メモはこのルールへの参照に置き換える。

**スコープ：memory は transient のみ。** memory が扱うのは self-evaluation log・self-evolution observation・reference に限る。cluster tally は transient だがここには入らない ── ホストにつき 1 つの計数面である一方 memory はワークスペースごとに解決するため、memory の外に置く（#2018。`rules/evolution/promotion-judgment.md` の Tally 節）。永続情報は memory に置かない。
...
**Consolidate トリガー：** 前回 consolidate から 2 週間経過で発火する。実行主体はトリガーが発火したセッションを保持する agent であり、外部の skill は名指ししない。pass の内容は、上の Entry Format の maintenance discipline を memory 集合全体へ次の順で当てることである。

1. 同種の entry を 1 本へ畳む（discipline の「重複は更新で処理」の適用）。畳んだ側のファイルは削除する
2. 陳腐化した entry を削除する（同 discipline）
3. 残った entry に合わせて `MEMORY.md` の索引を書き直す
4. `[[wikilink]]` が解決するか確認する。解決しないものは、entry を書くかリンクを外すかのどちらかに決める。未解決のまま残すのはどちらでもない
5. 実行を下記の 1 行として記録する

ここで外部ツールを名指ししないのは意図的である。同梱していない skill だけを経路として書けば、その skill を持たないホストでは手順がそこで止まり、止まりに出会った agent が他者の読めない即興の pass を入れる。

実行後は、索引 `MEMORY.md` の先頭に `**Last consolidate run:** <YYYY-MM-DD>` の 1 行として記録する。各 memory ファイルへは配らない。前回実行は memory 集合につき 1 つの事実であり、同じタイムスタンプを全ファイルへ複製すれば drift するのは複製側である（`rules/model/subtractive-structural-beauty.md` Core principle (A)）。行が無ければ未 consolidate と読み、トリガーは発火する。
...
**Self-Evolution Observation Format（自己進化 PR のマージ後検出サイクル）：** 保存先は `memory/self-evolution-observation.md`（workspace ローカル、gitignore）。cluster tally（memory の外に置く。`rules/evolution/promotion-judgment.md` の Tally 節）が起票前の観測であるのに対し、こちらはマージ後の観測である。起票トリガーは 3 つ。

- 自己進化 PR（`Evolution_Initiator_Autonomy` の起票経路）のマージ後の瞬間に、PR が変更した面のうち少なくとも 1 つが下の作成基準を満たすとき、その瞬間を持つエージェントがエントリを書く。瞬間と実行主体は `rules/operations/main-agent-procedures.md` Merge Execution の Post-merge moment が定める。expire 窓は PR のリスクに応じて選ぶ（既定 2 週間）。どの面も基準を満たさない PR にはエントリを作らず、満たさなかった条件をマージ済み PR へ 1 行のコメントで残す
- 短窓の miss エスカレーション：`rules/operations/operations.md` Post-L1-Merge Runtime Observation が `miss` 判定を出したとき、既定サイクルを待たずに親 AI が即座にエントリを書く。作成基準を満たすかどうかは問わない
- 短窓観測の繰り越し：同節の観測が、変更された規則がまだ実行時文脈に載っていないためマージ後の瞬間（`rules/operations/main-agent-procedures.md` Merge Execution の Post-merge moment）に開始できない場合、その瞬間を持つエージェントが繰り越しをこのエントリの `notes` へ 1 行として書き、後で観測を取ったセッションがその結果を 2 行目として追記する。そのためのフィールドは足さない。繰り越しそのものには verdict を入れない。エントリが無い PR では、2 行ともマージ済み PR へのコメントとして書く

作成基準は、変更された面が次の 2 つを両方満たすこと。

1. その面の application moment が、`expires` までの間に、ふつうの session の中で観察できる形で来る見込みがある
2. その面が壊れたことを検出する、実行される仕組み（テスト / CI）がない
