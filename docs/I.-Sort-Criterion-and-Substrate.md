...
**注 (2026-05-23, #1351)**: `rules/model/expansion-limit.md` / `rules/model/output-density.md` (新規 skill 化想定) / `skills/model-no-safety-net` は `rules/model/subtractive-structural-beauty.md` に意味で統合された (vector 統合)。以下の表中の関連行は本書作成時点 (#1280) の判定として歴史的記録のため残置。
...
`rules/model/` を literal に検証した結果、毎ターン必要と判定したファイル。暫定 11 を起点に literal 検証した結果、**substrate = 14 ファイル** で確定 (暫定 +3)。
...
確定数: **14 ファイル** (暫定 11 → +`layer.md` +`language-surface.md` +`character_Instance.md` の 3 を追加、`character_Instance.md` 配置上の特殊性 1 注記)。なお sub-issue #1285 で `layer.md` + `language-surface.md` が `liplus-coding-rule.md` に統合され、最終 substrate 集合は 14 ファイル維持 (置換 net 0、新規 `liplus-coding-rule.md` 1 ファイル追加で literal table 14 と整合)。
...
| `rules/model/expansion-limit.md` | output の expansion が 3 step を超えそうな瞬間 / 未要求の architectural redesign / future roadmap / 最適化提案を書こうとした瞬間 | rule 配置のまま (always-loaded) |
...
- `skills/model-no-safety-net/SKILL.md` / `skills/model-pair-review/SKILL.md` / `skills/model-requirement-deepening/SKILL.md` / `skills/model-review-output-partition/SKILL.md` / `skills/agentic-search/SKILL.md` — 対応する rule が `rules/model/` に存在しない独立 skill。発火モーメントを持つため真の skill。現状維持。`agentic-search` は #1220 で新設 (旧名 `model-agentic-search`、calibration + category OR trigger、 broad 探索軸の mechanical core)、#1380 (Phase 3 of #1217) で旧 `model-web-search-judgment` (Web 側消費規律) + `task-research-strategy` (親 governance) + `task-retrieval-orchestration` (親消費規律) を吸収して単一 skill `skills/agentic-search/` に統合済。
...
- substrate = **14 ファイル** (`rules/model/` から `absolute` / `boundary` / `foundational-invariant` / `language-definition` / `character` / `character_Instance` / `dialogue` / `axis-separation` / `layer-definition` / `layer` / `language-surface` / `role-separation` / `rule-policy` / `trigger-check-gate`。sub-issue #1285 完了後は `layer.md` + `language-surface.md` → `liplus-coding-rule.md` 統合により最終 substrate = 14 ファイル維持)
...
合計 = 14 + 8 + 3 + 7 = **32 ファイル** (planning 時点。`character_Instance.md` を substrate に含めたカウント。`rules/` 配下の実 file 数とは `character_Instance.md` 配置の特殊性によりずれが生じる)。
