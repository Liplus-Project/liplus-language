...
2. タスクレイヤー常時注入：Li+github.md から [Working with Issues]（[Source of Truth] + [Issue Management]）と [Research Strategy] を毎ターン注入する。
3. 通知取り込み：利用可能な通知源を1回 `inspect` し、[5. Notifications](5.-Notifications) の規則で前景一致した summary だけをホストへ渡す。empty/no-op は出力しない。
...
| `gh issue create` / `gh issue edit` | タスクレイヤーの Issue Format + Sub-issue Rules + Milestone Rules セクション | — |
| `gh issue view` / `gh issue list` / `gh api .*/issues` | タスクレイヤーの Issue Maturity + Sub-issue Rules セクション | — |
