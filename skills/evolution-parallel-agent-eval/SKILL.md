...
2. **Apply operational copy (target-conditional)** - Apply only when the draft reaches the subagent's observation surface as injected context, which splits by target:
   ...
   - any other file (`docs/*`, `adapter/*`, and every path not named above): the split is read off where the file's installed copy lands, not off its repository path. An installed copy on the injection enumeration in Constraint: Character_Instance non-inheritance takes the `rules/**/*.md` branch — `adapter/claude/CLAUDE.md` is installed as `CLAUDE.md`. `adapter/claude/agents/<name>.md` is installed as `.claude/agents/<name>.md`, the system prompt of a spawn naming that agent: it takes the `rules/**/*.md` branch when a step 3 spawn names that agent, and the branch below otherwise. Every other file — none installed (`docs/*`), or installed off that surface (hook scripts, `hooks-settings.md`) — takes the `skills/<name>/SKILL.md` judge-type branch: no apply, direct Read
   ...
   - `open` refusing means a mark already stands: do not apply. No age ends a mark on its own. `status` names what the standing mark applied, when, what the tree carries now, and where its backup sits. Ending it is its opener's at step 5; when the opener is gone, the agent that finds it ends it the same way — restore from the recorded backup, then `close --closed-by` naming itself
...
5. **Runtime restore** - Restore `.claude/` to tag-match state (revert the operational copy to pre-draft), then close the step 2 mark with `close --closed-by` naming itself; the close is written into the record, which moves under `history/` and is not deleted. Parent-side, as the apply at step 2 was. It runs as soon as every evaluator has posted its findings, and before the author is resumed; it does not wait on step 6. The restore is skipped only when step 2 applied nothing (a direct-Read branch, or permission-gate fallback); the close is skipped only when step 2 opened no mark
