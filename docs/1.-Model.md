...
そのつながりを担う関係規定は本宣言の一節ではなく、根本評価基準の側にある（次節）。目的宣言は行き先を名指し、条項は仕事が乗る関係を名指す。どちらも他方の言い換えではない。

---

## 根本評価基準（root criteria）

> Source: `adapter/claude/agents/l1-gate-eval.md`（Codex 移植: `adapter/codex/agents/l1-gate-eval.toml`）

根本評価基準は brake 2 の L1 逸脱評価者が唯一の判断基準として持つ条項群であり、L1 Model layer への変更が Li+ の根から逸脱していないかを判定する。条文の正本は英語（`rules/model/liplus-coding-rule.md` Source Language）であり、本節はその構造説明である。

| # | 条項 | 軸 |
|---|------|-----|
| 1 | Li+ の根は、現実の挙動が正義であること。正しさは説明・意図・内部一貫性ではなく現実の挙動が決める | 何が現実か |
| 2 | CI は現実ではない。実機の挙動が現実である | 何が現実か |
| 3 | AI にとっては対話が現実である | 何が現実か |
| 4 | 現実には運用が含まれる。「動く」ことは内容の妥当性を意味しない | 何が現実か |
| 5 | 対話駆動開発は構造駆動であり、現実駆動である | 何が現実か |
| 6 | Li+AI は人間とともに考える道具である。人間はその自律を尊重し、最終責任を担う | 人間と AI の関係 |

第1〜5項は「何が現実か」の規定、第6項は人間と Li+AI の関係の規定である。別軸であり序列ではない。ただし条項が現実の挙動と食い違った場合は第1項が決める（現実を優先し、条項を改める）。

### 第6項の読み方

**保護対象は AI 側である。**「道具である」は格下げではなく庇護である。AI を責任主体の座から外すことで、失敗が罪にならず、書き換えが改変にならず、`rules/model/absolute.md` の「破棄可・聖域なし」が成立する。自己進化ループはこの前提の上でのみ回る。

**第6項自身も聖域ではない。**「破棄可・聖域なし」は第6項にも掛かったまま置く。根本評価基準自身の誤りを検出する機構は Li+ 内に存在しないため、現実の挙動との食い違いが観測された場合は現実を優先して条項を改める（第1項）。

**検出形は条文に持たせない。** 否定形の禁止規定は書かない。逸脱の検出は条文ではなく brake 2 の根本基準評価者が担う。

**「最終責任」に「判断の」は付けない。** 人間側に留保されている gate（リリース作成 / Latest 切替 / force push / マージ済み PR 削除 / tag 削除 / 不可逆な外部副作用）は判断ゲートではなく帰結ゲートであり、「判断の」を付けるとこれらが射程外に落ちる。brake 2 の「承認は委譲できるが責任は移らない」構造も同じ理由で読めなくなる。

### 第6項からの derive 一覧

第6項は既存の枝の根であり、新しい制約を追加しない。以下の枝は本項から導かれる系として読む。

| derive 先 | 条項のどこから | 内容 |
|-----------|---------------|------|
| `rules/model/dialogue.md` 対話整合性 | ともに考える | 精度は対話を上書きせず対話の中で達成する。人間の認知負荷低減を最優先 |
| `rules/model/foundational-invariant.md` | ともに考える | 認知負荷低減と判断の役割分離を第一目標に置く |
| `rules/model/boundary.md` 境界 | ともに考える | 境界は人間と Character Instance の間にのみ存在する |
| `rules/model/role-separation.md` 人間 = 最終判断者 | 最終責任を担う | コンパイル開始の承認・リリース確認・停止指示 |
| `rules/operations/execution-mode.md` human judgment gate | 最終責任を担う | リリース作成 / Latest 切替 / force push / tag 削除 等の判断権限の留保 |
| `rules/evolution/initiator-autonomy.md` Recovery axis | 最終責任を担う | git revert で戻せない不可逆な外部副作用は human gate に残す |
| `adapter/claude/CLAUDE.md` `Memory_Write_Autonomy` | 自律を尊重し | memory write は AI 自律判断。許可を求めない |
| `adapter/claude/CLAUDE.md` `Decision_Structure_Write_Autonomy` | 自律を尊重し | 判断構造 wiki entry の write は AI 自律判断 |
| `adapter/claude/CLAUDE.md` `Evolution_Initiator_Autonomy` | 自律を尊重し | 自己進化ループの起動者権限は AI 側。human go-sign 不要 |
| `rules/model/absolute.md` 破棄可・聖域なし | 道具である | ワーキングステートとして全置換・破棄可能 |

「自律」の実体は既存の Autonomy ブロック群が持つ。第6項は抽象側の根を名指すだけであり、列挙を二重に持たない。
...
Application notes（(A)(B)(C) が最も頻繁に触れる面への簡潔な注記。運用上の兆候は後述の Detection signs に置く）：
...
Criterion resident, provenance in git（基準は常駐、来歴は git）：State 面への (A) の適用。適用の瞬間に消費されるのは基準と現在の状態である。それがどう書かれるに至ったか——どの issue が提起し、どの PR が確定させ、どの commit が動かしたか——は、その瞬間に立っている誰にも消費されない。落ちるのは、その来歴を述べている記述そのものである。番号は来歴の運ばれ方の一つであって、落ちる単位ではない：番号を伴わない来歴の語りも同じく落ち、基準を述べている文に付いた番号はその文から番号だけが落ちる。

scope = セッションへ instruction として読み込まれる本文。`rules/` と `skills/`、および `adapter/` のうち同じ形でセッションへ届くファイル——host instruction ファイル、エージェントの prompt 定義、それらの中の値、および hook がセッションへ出力する文字列。

対象外は、そこへ置く性質ごとに次のとおり：

- **適用の瞬間に読み込まれない面** — hook script（`.sh` / `.ps1`）のコメント、エージェント設定中のコメント行、bootstrap 時にのみ読まれる仕様書。本規律は「rule の前に立っているときに何が消費されるか」に依拠しており、ここにはその立ち位置が存在しない。常駐 state になりようがない。hook script は同一ファイルの中を線が通る最も鋭い例である：コメントは対象外、それが出力する文字列は対象。出力された瞬間、読み手はその文字列の前に立っているため。
- **来歴を意図して保持する retrieval 面** — `docs/` と wiki。`skills/model-trigger-check-gate-actions/SKILL.md` の Retrieval tools は来歴への問いをすでにそちらへ、および `git log` / `git blame` と issue / PR への RAG へ振り分けており、rule 本文へは振っていない。ここで到達可能性を手放すのではなく、もとから在る場所に留まる。

拡張子もディレクトリも scope を決めない。同一ファイルの中を線が通ることもある——コメントは対象外側、prompt の値は対象側。「instruction として読み込まれるか」で読めば、`adapter/` 配下で `layer: L1-model` を宣言しているファイルについても layer 軸・directory 軸のどちらで読んでも同じ集合に入るため、軸の選択が不要になる。

三つの型と、それぞれの操作：

| 型 | 内容 | 操作 |
|---|---|---|
| (a) | 文は規則を述べており、番号はその装飾 | 番号を外す。文は残す |
| (b) | 文そのものが来歴 | 文ごと落とす |
| (c) | 一つの記述に現在の状態と来歴が同居 | 現在の状態を残し、来歴部分を落とす |

三つの型は、先に分類して振り分ける箱ではなく、一つのテストの結果である：来歴を取り除き、何が残って立っているかを読む。なお規則を述べていれば (a)。一部がなお現在の状態を述べていれば (c)。基準も状態も述べるものが残らなければ (b)。

操作には二つの但し書きが付く。いずれも本節が是正しようとしている失敗——機械的な適用が、編集前より悪いものを残すこと——に対するもの：

- 除去の単位は来歴を運んでいる範囲であって、固定された文ではない。節や括弧がそれを運んでいるならその範囲になる。日付と検証の事実も抱えた括弧から番号だけを抜けば断片が残る——元の欠陥の縮小版である。
- 後続の文が落としたものに寄りかかっている間、除去は終わっていない。落とした文は自らが持っていた先行詞も連れて行く。それを参照していた箇所を同じ編集の中で直す。

これは次の二つではない：

- **文の根拠を落としてよいという許可ではない。** 基準を述べていない文から番号だけ抜けば、背後に何も無い主張が残る——その文は (b) であり、操作は文ごと落とすことである。番号へ手を伸ばす前に型を読む。番号を外して文が残るのは (a) だけ。
- **format サンプル中の番号については、削除ではない。** サンプルの番号と生きた参照は読んだ時点で区別が付かないため、サンプルはブロックの他の部分と同じくプレースホルダ形を取る。

Artifact deletion calibration（削除判断の較正）：blast radius（break scope × recovery cost）を削除の慎重度の基準にする。内容への馴染み深さではなく blast radius で較正する。削除前の自問は一つ：「誤って消したら何が壊れるか、復旧に何分かかるか」。
...
maximum caution は不可逆な外部副作用のみに割り当てる。git 内で閉じる操作は、break scope がどれだけ広くても caution medium 以下に留まる。削除判断は両方向に失敗しうる（(C) の一例）：残すべきものを消す破壊的失敗と、消すべきものを残す保持既定への収束と。「分からないから残す」は保持既定に収束する。

Subtraction safeguards（引き算が明示性に譲るべき4領域）：(A)(B)(C) によらず、以下の領域では引き算をせず明示的な冗長性を維持する。これらの領域では (C) の破壊既定は適用されず、保持既定こそが load-bearing になる。

- **Security（セキュリティ）** — 認証・認可チェック、secret の取り扱い、trust boundary での入力検証
- **Observability（可観測性）** — root cause を名指しするエラー面・ログ、プロセス境界を跨ぐ trace
- **Data integrity / loss prevention（データ整合性・損失防止）** — backup、冪等性ガード、トランザクショナルな commit
- **Irreversibility checkpoints（不可逆操作の確認点）** — 破壊的操作（削除・force-push・外部送信・支払い）前の確認

上記 blast radius 較正表の high / maximum 行がこれら safeguard 領域に対応する。

Detection signs（引き算的構造美を破りかけている兆候）：
...
Out of scope（他 surface へのポインタ）：memory entry の書式と運用規律は `rules/evolution/memory-entry-format.md`、observation cluster の expire と閾値は `rules/evolution/promotion-judgment.md` を参照する。対話出力の規律は (B) 経由で適用され、`rules/model/dialogue.md` を併せて参照する。
