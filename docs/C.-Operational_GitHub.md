...
-   正解や実装方法を書く必要はない
-   作成時は必要な見出しだけを書けばよく、3項目を最初から全部埋める必要はない
...
-   実装対象として扱う段階では、本文を以下へ収束させる
    ...
    -   前提
    -   制約
-   Issue の完了判断は本文項目ではなく、Issue 状態と PR/CI/release flow で扱う
...
Li+ では作業単位の完了を sub-issue の状態と PR/CI/release flow で管理できるため、チェックリストは原則不要。
...
-   実装単位の完了は子 issue の open/closed と派生 issue 分割で管理する
