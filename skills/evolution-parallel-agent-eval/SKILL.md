...
- **Effort floor = `medium`, set by the judge-type definition file**: a judge-type evaluator spawns under `brake-evaluator` (`adapter/claude/agents/brake-evaluator.md`, Codex port `adapter/codex/agents/brake-evaluator.toml`), whose `effort` is that floor. No host passes effort at the spawn call, so an evaluator spawned with no definition takes whatever the parent session is set to, and the definition is the only surface that fixes it; where a round spawns a probe-type evaluator, the floor is reached by raising the parent session instead (Procedure step 3). That definition names no `model` and no `tools`: the model floor above stays at the spawn call, and the no-write requirement stays on the prompt literal below
