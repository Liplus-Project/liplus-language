...
# clone. Everything the hook emits is the same in both modes. Case-sensitive,
# like the other Li+config.md keys read below.
LI_PLUS_MODE_VAL=""
...
# surface resolved through MEMORY_DIR until #2018 and reads its own path now, so a
...
# keeping one of its own, which is that split again. Same path as the claude
# port resolves, which is the point: one host, one tally.
#
