...
- **brake 1 (always)**: every self-evolution PR runs `skills/evolution-parallel-agent-eval` before the commit/merge gate. N=1 self-check is prohibited; minimum N=3. The eval runs a **single round**: the parent adjudicates the findings against the source, revises, and takes the revised draft to self-review — no re-verification round on the revised draft. Sample count and round count are separate axes; the N=3 floor is unaffected by the round cap (`#1296`'s basis is sample count).
