...
scope = セッションへ instruction として読み込まれる本文。`rules/` と `skills/`、および `adapter/` のうち同じ形でセッションへ届くファイル——host instruction ファイル、エージェントの prompt 定義、およびそれらの中の値。

対象外は、そこへ置く性質ごとに次のとおり：

- **適用の瞬間に読み込まれない面** — hook script（`.sh` / `.ps1`）のコメント、エージェント設定中のコメント行、bootstrap 時にのみ読まれる仕様書。本規律は「rule の前に立っているときに何が消費されるか」に依拠しており、ここにはその立ち位置が存在しない。常駐 state になりようがない。
