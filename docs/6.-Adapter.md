...
- promotion candidates（memory → Li+ source）：自己評価ログの `(root_cause, domain-tag)` 重複、最近（7日以内）の memory 追記、memory header と Li+ source のキーワード重複（section key: `promotion_candidates`）
  - `MEMORY_DIR` の解決規則：候補ディレクトリは優先順に走査するが、採用条件は「ディレクトリが存在すること」ではなく「消費側が読むファイルを1つ以上持つこと」。marker 集合は全消費者の和（`self-evaluation_log.md` / `feedback.md` / `project.md` / `self-evolution-observation.md`）であり、特定の消費者に寄せない。存在するだけの空ディレクトリが上位候補にあると、下位候補に実体があっても全消費者がまとめて黙るため。どの候補も marker を持たない場合は未解決のままとし、各消費者が個別のファイル存在チェックで silent skip する（従来と同じ帰結）
