...
- **永続情報の昇格先は 4 系統**：Li+ 正規ルール（`rules/` / `skills/`）／ `docs/` ／ wiki（`docs/Decision-Log.md` index 配下）／ 削除。詳細は `rules/evolution/memory-entry-format.md` の Escalation paths を参照
...
- `docs/` = プロジェクト判断 / 仕様レベル
- wiki（`docs/Decision-Log.md` index 配下、kebab-case `<topic>.md`）= 判断記録
- 削除 = 撤回 / 陳腐化 / Li+ 既昇格済み
