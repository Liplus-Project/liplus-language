...
**`auto` / `semi_auto`**：`PR open + CI green` で停止する。サブエージェントはセルフレビューを実行も投稿もせず、そこで報告して終える。以降は親が持つ——`rules/evolution/initiator-autonomy.md` Two-stage brake の定める位置（CI green の後・merge ゲートの前）で brake 1（PR が L1 Model Layer ソースに触れる場合は加えて brake 2）を走らせ、指摘を裁き、セルフレビューし、マージする。
