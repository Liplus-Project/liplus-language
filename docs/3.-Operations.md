...
Li+operations.md は `.claude/rules/` 経由で常時コンテキストに存在する（compaction を生存）。PostToolUse hook が focus pointer として、ブランチ作成、コミット、PR、マージ、リリース等の操作タイミングで該当セクションへの注意喚起を行う。

Issue Format、Issue Maturity、Sub-issue Rules、Milestone Rules は Li+github.md から Li+operations.md に移動された。これにより issue 操作時の focus pointer も Li+operations.md 内で完結する。
...
- メインエージェントが PR diff を issue 要件と照合してレビューする（Li+github.md#PR_Review_Judgment 参照）。
- pass → マージへ移行。
