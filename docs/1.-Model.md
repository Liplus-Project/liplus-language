...
Li+ ソース本文（`rules/` / `skills/` / `adapter/`）が述べるのは振る舞いである。理由は判断記録（wiki）が持ち、本文はそこを名指すだけにする。
...
**行き先は移設であって削除ではない。** 理由に当たる語句の行き先は本節から読み、impression literal に当たる語句の行き先は `skills/evolution-impression-literal-detection/SKILL.md` から読む。Judgment record: `source-body-behavior-only-reasoning-relocates`。

**ポインタは必須。** 理由を抜いた跡には、それを持つ wiki エントリを名指す `Judgment record: <slug>` を残す。ポインタ無しの抜き取りは禁止。Judgment record: `source-body-behavior-only-reasoning-relocates`。
...
skill 本文は 2026-09-02 に `rules/model/liplus-coding-rule.md` Body States Behavior を適用済みであり、振る舞いのみを載せる（#1849、386 行 22,594 B → 223 行 10,576 B、−53.2%）。剥いだ理由（OR 構成／work-mode damping／spin-wheel 抑制の各根拠、`Three roles of the judging AI` 節、Web 側 baseline reminder、Tier/Stage の違いの説明）は判断記録 `agentic-search-distilled-body-measured-equivalent` が保持する。本節は記録面であり同規則の scope 外——理由を持ち続けてよい。

本件は Body States Behavior の適用結果を**外部 probe で測った**最初の例である。#1848 の段2構成で A 腕 = full 版 / B 腕 = 蒸留版を各 3 本走らせ、`description` を 1 バイト違わず同一に保ったうえで 5 問全問一致・差ゼロを得た。ただし probe は `Three roles` 節・Web 側 baseline reminder・Tier/Stage 説明段落に触れていないため、結論は「この 5 問の範囲で同じ振る舞いをした」までである。本文に `Judgment record:` ポインタは置いていない。brake 1 の評価者3体が全員これを literal 違反として検出しており、その読みは `rules/model/liplus-coding-rule.md:39` のとおりで正しい。疑われたのは literal のほうで、Master の判断（2026-09-02、dialogue）「`Judgment record:` ポインタはいらない機能だね」を受けて要件撤廃の #1852 が起票された。ポインタを足さなかったのはその判断による。当初 PR 本文に書いた「足せば測定された本文でなくなるため」という理由づけは規則側に根拠を持たない構成であり、撤回する。
