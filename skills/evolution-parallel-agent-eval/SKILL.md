...
3. **Parallel subagent spawn** - Select N, M, P per draft nature and spawn in parallel. Default `N=3, M=all axes, P=1`, total invocation = 3; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   Three things go into the prompt alongside the material:
...
6. **Judgment** - consistent -> push the spec change toward implementation. partial / negative -> the parent adjudicates each finding against the source, revises the draft, and takes the revised draft to self-review. Or abort. **Single round**: steps 2-4 produce one verdict per draft, and the revised draft ships without re-verification (see Non-scope: what the single-round cap gives up). The cap bounds re-verification, not retries of a round that produced no verdict — step 2's refused `rules/*` apply leaves no verdict, so re-running from a session that can apply IS the round. Its other two branches (`skills/*` direct-Read fallback, `rules/*` proceeding on a reduced-confidence deviation record) produce a verdict and are unaffected
...
- **N=1 prohibited, minimum N=3**: One trial is the source of overconfidence — `#1296` observed the conclusion reverse from N=1 positive to N=3 = 1 positive + 2 partial-negative. Reference Design Dimensions' `subagent_count` for N and run at minimum 3
...
- **`skills/task-subagent-delegation/SKILL.md`**: This method's subagent spawn is a special case of delegation (purpose: gather evaluation data, not delegate implementation)
