...
The assignee is set at the delegation moment, by the parent, in the same act as the spawn: `gh issue edit {issue_number} -R {owner}/{repo} --add-assignee "@me"` (parent and delegated subagent authenticate as the same GitHub actor, so `@me` names the account that will hold the issue). It is not a label and does not join the state-machine subset — `in-progress` stays the subagent's, raised when its work starts (`skills/task-subagent-state-labels/SKILL.md`). Do not pull that half forward with it: a running-claim set before the runner exists survives a subagent that dies mid-investigation and locks the issue against everyone (false lock), while an assignee claims only who it was handed to. The asymmetry is what the split rests on. What the forward move buys is the removal of a read: the delegation moment is a point the parent already occupies, whereas `Work start` is read by the executor and #1714 measured that reading arriving late. Additivity is unchanged — `--add-assignee` does not displace a prior assignee. The full argument and the reading rules for the field are held at that skill's Actor axis.
