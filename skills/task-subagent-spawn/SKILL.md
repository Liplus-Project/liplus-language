...
- **For a spawn whose bare behavior is the observation target, set the model at the spawn call; do not give it an agent definition file that pins one.** In Claude Code the agent file body replaces the subagent's system prompt, so a frontmatter `model:` pin cannot be applied without also replacing identity — which mutates the probe-type observation target brake 1 depends on. The per-call parameter changes neither context nor identity. The reason reaches exactly as far as that observation target: the brake 1 evaluators, whose bare behavior is what the eval reads. It reaches no other spawn, and the Codex evaluator has carried its own definition file throughout without the reason applying to it. Outside the evaluators a definition file is permitted, and the implementation delegate carries one (next bullet).
- **Delegations under `skills/task-subagent-delegation/SKILL.md` spawn the Li+ implementation delegate, not the host's built-in general-purpose agent.** Claude Code: `subagent_type: implementer` (`adapter/claude/agents/implementer.md`). Codex: the `implementer` agent (`adapter/codex/agents/implementer.toml`). The definition exists to pin thinking effort — `effort: high` on the Claude port, `model_reasoning_effort = "high"` on the Codex port — which no spawn-call parameter can set. It names no model on either port, so the `model` bullet above still holds unchanged: omission at the spawn call is what inherits the parent tier, and the definition does not take that over. The body is written to stand on its own, because whether the host layers it over a base prompt or replaces one is not settled; a body that reads as a delta against an unmeasured base is what that unsettled state would break.
