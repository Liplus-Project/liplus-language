...
- Question 2 has no current case. `adapter/*/agents/dialogue-evaluator.*` was the one, and it was resolved on the write side rather than by widening the region: the Character_Instance literal it interleaved with its owned criteria now arrives through the invocation prompt, so what remains in the body is Li+-owned throughout and one region covers it. Read question 2 as live all the same — it is the question that decides a file, not a record of which file once failed it, and a No still means Create-only with that file's propagation gap left open.
- The move that resolved it is the one available to an author who lands on a No: take the user-owned content out of the body through a route that already exists, rather than drawing the region wider. Where no such route exists, the No stands.
