...
`skills/task-subagent-delegation/SKILL.md` に基づく委譲は、ホスト組み込みの汎用エージェントではなく Li+ が配る実装委譲先を spawn する。Claude Code では `subagent_type: high`（`adapter/claude/agents/high.md`。役 literal は上記のとおり spawn プロンプトが運ぶ）、Codex では `implementer` エージェント（`adapter/codex/agents/implementer.toml`）である。

Claude 面で定義ファイルを置く理由は思考 effort の固定にある。Claude Code では effort が spawn call の引数に存在せず、`high.md` frontmatter の `effort: high` が固定する。Codex 面は spawn call で `reasoning_effort="high"` を渡し、`implementer.toml` に `model_reasoning_effort` を置かない。定義側の値は per-launch の解決値を上書きするためである。

`model` はどちらの面にも書かない。親モデルの継承は spawn call での省略が担っており（`skills/task-subagent-spawn/SKILL.md`）、定義ファイルはその軸を引き取らない。親のティアが変わったときに追従する性質をそのまま残すためである。

Claude 面の本文（`high.md`）は役を持たないため単体で成立する形の懸念自体が消える。役 literal（`skills/task-subagent-prompt/SKILL.md` 側）は単体で成立する形で書く——ホストが基底プロンプトの上に重ねるのか置き換えるのかは判別が付いていない（2026-09-11 の実測で判別不成立）。未測定の基底に対する差分として読ませる本文は、その未決着がそのまま壊す形になる。
...
`model` は effort の軸と独立して spawn call で明示し、sonnet 級の床を担う（`skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Model floor）。Claude の effort 定義にも Codex の brake spawn にも `tools` 制限を加えない。評価者のツール権限を絞らない判断と、read-only を `tools:` で表現しない判断は同 skill の Non-scope が持つ。
...
- implementation delegate と dialogue evaluator は `reasoning_effort="high"` を指定する。
