...
Relation axis, separate from the state subset: which issue the work waits on is the GitHub issue dependency (`blockedBy` / `blocking`), not a label. A state label says the work is stopped; the dependency says on what. `blocked` stays the human-input state and is not derived from the dependency graph. `waiting` on a dependent issue is a state as well, and the issue waited on is recorded as a dependency. Who records it = `rules/operations/main-agent-procedures.md` Issue format.
