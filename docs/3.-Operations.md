...
ブランチと issue ツリーの対応は「1親 issue = 1ブランチ」である。`gh issue develop` でブランチを作るのは親 issue のみ。sub-issue（子・孫・ひ孫…）は親のブランチ上にコミットする。sub-issue に独自ブランチは作らない。別ブランチが必要なら別の親 issue を立てる。

親エージェントが checkout した状態でサブエージェントを起動する。サブエージェントは checkout も cd もしない。

PR マージ時にリンクされた親 issue が自動クローズされる。全 sub-issue が同一ブランチ上で完了してからマージするため、親の自動クローズは意図通りである。`gh issue develop` のリンクは1 issue につき1ブランチのプラットフォーム制約があり、このルールはそれに沿っている。
