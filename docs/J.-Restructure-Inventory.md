...
**注 (2026-05-23, #1351)**: `rules/model/expansion-limit.md` / `skills/model-output-density` / `skills/model-no-safety-net` / `skills/task-deletion-impact` は `rules/model/minus-aesthetic.md` に意味で統合された (vector 統合)。以下の関連行 (L47 expansion-limit / L48 output-density / L65 no-safety-net / L114 task-deletion-impact) は本書作成時点 (#1282) の判定として歴史的記録のため残置。
