...
- brake 1 の **effort の床は `medium` であり、モデル床とは独立に解決する**。モデル床がモデル軸しか守っていなかったことが実測で出ている——2026-09-13 の Claude 実測では、定義ファイルを持たない評価役が親セッションの effort を継承し、本 workspace の 2 セッション全件でほぼ `low` で走っていた。値は暫定であり（Master 判断、2026-09-13）、`low` の外し方——正解の literal に到達しながら「保護すべき rationale」と判断して finding から外す、探索ではなく境界判断が保護側へ寄る形——に対して最小の変更で当たる位置として 1 段だけ上げた。`medium` / `high` の効き方は未測定であり、未測定の 2 段上げは根拠を持たない。Claude は judge 型の `brake-evaluator` 定義が `effort: medium` を置き、probe 型は親セッション自身の effort を上げて床へ届く。Codex は judge / probe とも定義ファイルを選ばず、各 spawn call で `reasoning_effort="medium"` を明示する。Codex の agent TOML に `model_reasoning_effort` は置かない——定義側の優先順位が per-launch 値を上書きするためである。sonnet 級の `model` は別の per-call 軸として維持し、対応しない effort 値への fallback は推測しない。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Effort floor と Procedure step 3（#1968 / #1973）
