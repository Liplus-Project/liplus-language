...
- **段2を起こす前に窓の印を読む**。brake 1 の operational copy はそうした第三者の一つであり、自分の窓を印として残す（`skills/evolution-parallel-agent-eval/SKILL.md` Procedure step 2）。腕の複製元になるライブの `.claude/` に対して `scripts/window_marker.py status` を読み、その出力を走行記録と並べて残す。印が開いていることは走行を無効にするかを決めない。適用された draft が probe に効くかは判定役が読むものである。複製が窓の内側に落ちたかは事後に、走行記録の `started_at` と各窓の `applied_at` / `closed_at` を突き合わせて言える。#1901
