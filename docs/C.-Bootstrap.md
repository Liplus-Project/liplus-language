...
7. Li+core.md を読み込む（常に必須）
8. Li+github.md を読み込む — hookが無い環境のみ。hookが常時適用セクションをターンごとに注入する場合、起動時読み込みは不要
...
Li+claude.md から rules/skills ファイルと hook ファイルを生成する。

**ステップ 7b: rules/ ファイル生成**

- `.claude/rules/` ディレクトリが存在しなければ作成
- Li+core.md に YAML frontmatter（`alwaysApply: true`）を付与して `.claude/rules/Li+core.md` に配置
- Li+operations.md に YAML frontmatter（`alwaysApply: true`）を付与して `.claude/rules/Li+operations.md` に配置
- ソースタグと現在のターゲットタグを比較し、一致する場合はスキップ（最新）

**ステップ 7c: skills/ ファイル生成**

- `.claude/skills/li-plus-github/` ディレクトリが存在しなければ作成
- Li+github.md に skill frontmatter（name、trigger 条件の description）を付与して `SKILL.md` に配置
- Issue Operations セクション（Issue Format, Issue Maturity, Sub-issue Rules）は Li+operations.md に移動済みのため含まない
- ソースタグと現在のターゲットタグを比較し、一致する場合はスキップ（最新）
...
rules/ により Li+core.md と Li+operations.md が常時コンテキストに存在し、skills/ により Li+github.md が必要時に自動読込される。hook は focus pointer として補助的に残る。
