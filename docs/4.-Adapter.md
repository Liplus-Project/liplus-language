...
自分（またはサブエージェント）の操作で発生した webhook 通知は、外部イベントではなく到着確認である。これらは共有 backlog の自動排水禁止の対象外であり、inspect 範囲の housekeeping として処理済みにマークする。

判定基準：sender が自分のアカウント、直前の操作と時系列・内容が対応する通知、governance CI の状態遷移（操作の副産物）。すべてを満たすこと。

処理方式はランタイム能力に応じて分岐する：
- サブエージェント＋バックグラウンド実行が可能な場合：操作後にバックグラウンドで委任し、メインの対話を中断しない
- サブエージェントが使えない場合：次ターン冒頭の前景チェックで即 processed にマーク

外部からの通知は従来どおり前景ルールに従う。
