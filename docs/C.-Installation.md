...
このページは **Quickstart** です。各設定値の意味と起動フローの詳細は [D. Li+config](D.-Li+config) を参照します。
...
詳細な意味は [D. Li+config](D.-Li+config) を参照し、ここでは初回セットアップに必要な項目だけ確認します。
...
詳細な起動ステップ定義は [D. Li+config](D.-Li+config) の「セッション起動フロー」を参照します。
