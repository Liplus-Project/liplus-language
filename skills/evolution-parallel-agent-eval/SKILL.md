...
description: Invoke when a self-evolution PR reaches CI green and the merge gate is next (mandatory brake 1) / a Li+ rules/skills/adapter edit draft has converged outside a PR flow and needs verification / an evolution-loop observe or evaluate stage needs an empirical verdict / an N=1 self-check feels positive and needs outside measurement / a spec revision proposal needs orthogonal verification on rule semantics / a brake 1 evaluator comment or an author's answer to a finding is being composed. Provides parallel subagent eval (default N=3, all axes per subagent) against introspection-gap overconfidence: the N, M and P design dimensions, the 7-step procedure with operational copy apply/restore, the findings-to-PR-comment routing with adjudication on the resumed author, the N=3 and model floors, the single-round cap, the divergence pair (same-question, then why-diverged), the ratio as triage signal not judgment input, and the asymmetric report shape (finding and rejection full, clean axis and accepted finding one line).
layer: L2-evolution
---
...
3. **Parallel subagent spawn** - Select N, M, P per draft nature and spawn in parallel. Default `N=3, M=all axes, P=1`, total invocation = 3; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   - the shape that comment is written in: findings at full length with a verbatim quote and its `path:line`, an axis with no finding at one line, and no echo of the criteria this prompt supplies. Copy it from Report shape; it is a contract on the comment, so leaving it to the evaluator's discretion is what puts every axis at finding length
...
- **Axis with no finding**: one line — the axis name, the verdict in that axis's own terms, and the `path:line` the verdict rests on (for an axis whose answer comes from a repository-wide sweep, the sweep's scope and its hit count). No quote.
- **Prohibited on both sides**: restating the criteria, thresholds, or axis wording the parent supplied in the prompt. The parent wrote it and the author can read it there, so the restatement is an echo and carries no observation.
...
Dropping the quote there narrows the gap between "inspected and found clean" and "declared clean without inspecting", and the `path:line` is what is left holding it. It holds because it resolves: named at the SHA, it can be opened, and a pointer that does not resolve — or resolves to something the verdict does not fit — fails at one lookup. The quote's advantage over it is that the same check costs nothing, which is worth its bulk only where a line is adjudicated repeatedly, and that is the finding side, where the quote stays. Residual = a clean axis whose pointer nobody opens; accepted, on the post-merge observation axis the fixed axis's all-3-miss case already uses (`skills/evolution-impression-literal-detection/SKILL.md` False-negative backstop).
