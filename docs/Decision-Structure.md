...
| [`human-ai-tool-relation-root-criterion`](https://github.com/Liplus-Project/liplus-language/wiki/human-ai-tool-relation-root-criterion) | 人間と Li+AI の関係を L1 根本評価基準の第6項として明文化する判断（#1737 / PR #1738、2026-08-13、Master 決定）— 条文（英語正本）は `Li+AI is a tool that thinks together with the human. The human respects its autonomy and carries the final responsibility.`。**保護対象は AI 側**で、「道具である」は格下げではなく AI を責任主体の座から外す庇護（責任主体として扱えば失敗が罪に、書き換えが改変になり `rules/model/absolute.md` の「破棄可・聖域なし」が成立しない＝自己進化ループの前提）。既存5項がすべて「何が現実か」の規定であるのに対し本項は人間と AI の関係の規定で、**新しい制約を追加せず既存の枝の根を名指すだけ**（`dialogue.md` 対話整合性 / `foundational-invariant.md` / `boundary.md` ← ともに考える、`role-separation.md` 人間=最終判断者 / `execution-mode.md` human judgment gate / `initiator-autonomy.md` Recovery axis ← 最終責任を担う、Autonomy ブロック群3種 ← 自律を尊重し、`absolute.md` 破棄可 ← 道具である）。位置は現実軸5項の末尾で、第1項「Li+ の根」の位置は不変（条項が現実の挙動と食い違えば第1項が決める＝本項も聖域ではない）。制約 = 検出形（否定形の禁止規定）は条文に持たせず検出は brake 2 の評価者が担う／「判断の最終責任」とは書かない（人間側 gate は判断ゲートではなく帰結ゲートであり、「判断の」を付けると射程外に落ち、brake 2 の「承認は委譲できるが責任は移らない」構造も読めなくなる）／「自律」の実体は既存 Autonomy ブロック群が持ち抽象と列挙の二重定義を作らない。**自己審査構造を受容** — brake 2 自身の判定基準を書き換えるため評価者が新基準で自分の変更を審査する（外部審査面を作る案は却下：根本基準の外に基準を置けばそちらが実質の根になり問題が一段上へ移動するだけ）。却下 = 既存の枝への分散記述に留める案（根が無いと各枝が独立規定に見え継承元が読めない）／条文への検出形併記（検出責務の二重化）／`rules/model/` 側への条文書き写し（二箇所 drift、ポインタで代替）。`l1-brake2-root-criteria-evaluator` に depends on、`master-role-as-client-architect` に relates to |
