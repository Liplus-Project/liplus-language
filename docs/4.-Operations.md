...
- **docs/ 所有ファイル**（大文字 + 数字 prefix + `Home.md` + `_Footer.md`）: docs/ が正本、Wiki は同期先。同期後は byte 単位で一致する。`docs/Decision-Log.md`（判断記録レイヤーの index）は通常の docs/ 所有大文字ファイルとして扱う（docs/ 側は `adapter/claude/hooks/on-session-start.sh` が cold-start synthesis 素材として head を emit、Wiki 側はナビでの可視化）
- **Wiki 専属ファイル**（小文字 kebab-case `[a-z]*.md` の判断記録 + `_Sidebar.md` 等の Wiki 専用ナビ）: Wiki が所有、docs/ に counterpart を持たない、同期は preserve する。判断記録 entry は順序 prefix を持たず、順序は `docs/Decision-Log.md` と `_Sidebar.md` で明示する
...
- 初期 docs/ に `Home.md` / `_Footer.md` / 正規の大文字 + 数字 prefix ファイル（`docs/[A-Z]*.md`、`docs/[0-9]*.md`）を配置。`docs/Decision-Log.md` は判断記録レイヤーの index として含める
...
3. 選択 wipe: docs/ 所有ファイルのみ Wiki から削除、Wiki 専属エントリは残す:
   ...
   ※ `[a-z]*.md`（判断記録）と `_Sidebar.md` は明示的に対象外。`Decision-Log.md`（大文字 `D` 始まり）は `[A-Z]*.md` でカバーされる通常の docs/ 所有ファイル
4. docs/ をコピー: cp docs/*.md {tmpdir}/
   ※ docs/ は大文字 + 数字 prefix + `Home.md` + `_Footer.md` + `Decision-Log.md` のみ。判断記録 entry は wiki 専属なので、この cp は wiki-only エントリを再導入しない
