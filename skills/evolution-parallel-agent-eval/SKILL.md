...
- **`skills/task-subagent-delegation/SKILL.md`**: Derived use from the delegation axis - this method's subagent spawn is a special case of delegation (purpose: gather evaluation data, not delegate implementation). That skill's general delegation model default (`sonnet`) and Parallel-Width Cap live there; this skill's opus-class model floor (Constraint) and N / M / P width (Design Dimensions) are each an explicit, reasoned exception to those general defaults — the width exemption is stated in that skill's Parallel-Width Cap section
