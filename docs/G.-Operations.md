...
- 要求仕様書は issue 群から採用された目的・前提・制約を固定する正本であり、実装後の追従物ではない
...
有効ラベルの辞書は [C. Operational GitHub](C.-Operational_GitHub) を正本とし、このページでは**運用ルールと廃止履歴**だけを扱う。
...
- タイプラベル・成熟度ラベル・ライフサイクルラベルの定義は [C. Operational GitHub](C.-Operational_GitHub) を参照
