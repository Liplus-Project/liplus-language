...
| [`rules-hold-criteria-not-provenance`](https://github.com/Liplus-Project/liplus-language/wiki/rules-hold-criteria-not-provenance) | `rules/` と `skills/` の本文に規則の来歴を運ぶ番号（issue 番号 / PR 番号 / commit SHA、およびそれらの形をしたサンプル値）を書かないと確定させた判断（#1721 / PR #1722、2026-08-10、Master 判断）— 適用の瞬間に消費されるのは基準と現在の状態であり、来歴を運ぶ番号は常駐すべき state ではない。到達可能性は `git log` / `git blame` と issue / PR への RAG がすでに引き受けている。但し書き 2 件 = 番号を剥がすのは基準文が残る箇所に限る（基準の根拠を落とす許可ではない）／ format サンプル中の番号は削除ではなくプレースホルダ化。却下 = 実測記録としての番号を例外として残す案（AI の初期主張。閉じ手は基準文であって事例ではない）。実測は 65 件 / 17 ファイル。`li-plus-always-on-footprint-load-bearing` に conflicts with（同 entry の一般命題「always-on に乗っている内容は load-bearing」と literal で衝突するが、同 entry が実際に論じた 3 つの構造的移設案の却下理由は動かない。未解決点 = 一般命題側への scope 限定の要否）、`liplus-structure-as-retrieval-surface` に depends on、`always-on-duplication-removal-direction` に relates to |
