...
| `resume` / `clear` / `compact` / `fork` | 作業 context は連続のため diff-only 評価は行わず、cold-start rule literal だけを再 anchor。state file は更新しない |
...
**Claude Code**：Agent ツールの `SendMessage` で、spawn 時に返るエージェント id または名前を宛先にして再開する。新しい `Agent` 呼び出しは cold start であり再開ではない。フェーズ1の spawn で得た id を保持すること。失えば、再開が保とうとしている実装文脈そのものを失う。

**Codex**：`resume_agent` ツールで、保存済み rollout からエージェントを復元する。同じくフェーズ1の id を保持する。`fork_turns` は spawn call の引数であってここには関係しない——再開は構造上エージェント自身の保存済み context を引き継ぎ、それがこのフェーズの依拠する性質である。`resume_agent` が使えない場合は、著者役のサブエージェントを新規 spawn し、issue body・PR diff・PR コメントから再構成するフォールバックを許容する。役割は変わらないため、注入する権限境界も変わらない。
