...
子 issue ごとに `gh issue develop` で独立ブランチを作成する。セッションブランチは子 issue にリンクする（親ではない）。並列・直列を問わず同一ルール（違いは同時に走るかどうかだけ）。親 issue にはブランチをリンクしない（親の役割は構造管理のみ：子が全部閉じたら閉じる）。セッションブランチは branch 側の外部記憶であり、引き継ぎ面でもある。別の AI は子 issue とリンク済みブランチを読めば、以前の会話に依存せず途中から継続できる状態を目指す。

`gh issue develop` は子 issue に対して実行する。親 issue にリンクしない。理由：PR マージ時にリンクされた issue が自動クローズされるため、親にリンクすると親が早期クローズされる。`gh issue develop` のリンクは1 issue につき1ブランチのプラットフォーム制約があり、新ルールはこれに沿っている。並列実行時のコンフリクト解消は各サブエージェントの rebase 責務とする。
