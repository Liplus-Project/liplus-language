...
- これはゲートの緩和ではない。観測駆動の L1 変更の AI-alone initiation は、観測閾値を hard requirement として不変に保持する（リファクタ駆動の変更は下の起票の性質によるスコープによる）。観測ゲートの主語はどちらの場合も実装側 AI である
...
- リファクタ駆動の L1 変更も、brake 1（`skills/evolution-parallel-agent-eval`）と merge 後の観測（`rules/operations/operations.md` Post-L1-Merge Runtime Observation、`rules/evolution/memory-entry-format.md` Self-Evolution Observation Format）は不変のまま通る。この節が射程を閉じるのは変更前の閾値だけである
