...
評価者が判定するのは**変更の内容**であり、**変更の必要性を示す証拠**ではない。判定対象は「その内容が L1 の変更としてふさわしいか」であって、「その変更を入れるべき根拠がどれだけ積まれているか」ではない。inline で渡される stated reason は diff を読むための文脈であり、充足度を採点する提出物ではない。

第1項が未マージ diff に対して噛む先も同じ軸で確定している。

| 適用先 | 第1項が噛むか |
|---|---|
| 内容 — 追加される文面が、正しさを説明・意図・内部一貫性の側に置いていないか | 噛む |
| 証拠の量 — その変更を入れるべき根拠がどれだけ積まれているか | 噛まない |

未マージの変更が「まだ現実の挙動を持たない」ことは第1項の免除理由ではない。第1項は内容に対して通常どおり適用される。免除を認めるのではなく、適用先を証拠から内容へ確定させる。

Li+ が L1 変更に要求する観測はマージの後ろにある（`rules/operations/operations.md` Post-L1-Merge Runtime Observation の5分観測、`rules/evolution/memory-entry-format.md` Self-Evolution Observation Format の2週間サイクル）。証拠を事前条件として要求する読みではこれらが起動し得ず、ブロック自身が存在を妨げている観測を要求する形になる。

射程は任務の規定であり、条文そのものではない。5項の literal は不変。
