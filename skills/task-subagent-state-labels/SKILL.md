...
- Entering brake adjudication (`auto` / `semi_auto` phase 2) → switch `review-pending` → `in-progress` before starting. Reading the findings, adjudicating them and pushing what was accepted is work running. The trigger is the phase, not the mechanism that put you in it: a true resume and a cold spawn under the reconstruction fallback (`skills/task-subagent-prompt/SKILL.md` Resume-phase authority boundary) both enter phase 2 and both fire this transition. Entering a later brake round fires it again, on either path, for the same reason.
