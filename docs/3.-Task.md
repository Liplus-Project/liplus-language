...
サブエージェントはブランチ作成、実装、コミット、プッシュ、PR 作成、CI ループ、セルフレビュー、マージを実行する。
...
判断の前後でメイン自身が実行する行為——セルフレビューの formal record 投稿とマージ手続き——は、この面にも禁止された面にも無く、常駐の `rules/operations/main-agent-procedures.md` が正本である。本 skill が持つのは判断であり、判断が解放する行為はそちらが持つ。
