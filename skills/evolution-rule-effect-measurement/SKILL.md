...
The run is not mandatory. Measurement consumes external budget (Running stage 2), so a run that cannot be taken does not hold the merge gate — proceed to brake 1 without it. What holds instead is the record: write `unmeasured` with its reason — spend limit, outside the firing condition, nothing in the diff to raise a probe over, or the brake 1 window still open after the bounded wait (Running stage 2) — into the parent's self-review record (`rules/operations/main-agent-procedures.md` Self-review formal record). Never leave it blank and never record it as a negative result. A blank is read as no problem, which makes the absence of a verdict work as a verdict. That record is written after brake 1 has exited, so nothing in it reaches an evaluator.
