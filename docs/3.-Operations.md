...
Li+ の挙動や運用ルールに効く判断は、番号付き要求仕様と対応する運用文書へ固定する。補助メモや実験ログを残してもよいが、それを正本にしない。
...
PR 作成直後に自動起動する。人間の指示は不要。CI ループが完了するまで PR タスクは完了しない。
...
CI PASS → CODEOWNERS 経由でレビューリクエスト自動送信。
CI FAIL → 修正してリコミット。ループ安全閾値（3回）を超えた場合は issue コメントに外部化し、人間に委ねる。
...
- `APPROVED` → auto-merge に任せる
- `CHANGES_REQUESTED` → レビューコメントを読んで修正し CI ループを再起動する

---

## マージとクリーンアップ

auto-merge を使用する。PR 作成直後に auto-merge を有効化する。
