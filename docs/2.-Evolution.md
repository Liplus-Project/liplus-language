...
- skill 本文（`skills/evolution-parallel-agent-eval/SKILL.md`）は 2026-09-01 に `rules/model/liplus-coding-rule.md` Body States Behavior を適用済みであり、振る舞いのみを載せる（#1845、49,217 → 39,796 B）。剥いだ理由のうち既存 entry が持たない分は判断記録 `parallel-agent-eval-body-carries-behavior-only` が保持する。本節は記録面であり同規則の scope 外——理由を持ち続けてよく、写るのはポインタだけである
