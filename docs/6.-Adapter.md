...
出力順序（bootstrap 済みの通常セッション）：`Li+ update status` → `Li+ language contract` → `gh install`（該当時のみ）→ cold-start 素材（matcher 依存）。
