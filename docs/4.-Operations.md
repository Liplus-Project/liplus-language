...
- body 行が解決するのは host workspace に対してである。`Workspace_Language_Contract`（`adapter/claude/CLAUDE.md` / `adapter/codex/AGENTS.md`）はその到達範囲がどこで止まるかの正本であり、述べているのは止まる位置だけである（`They do not change LI_PLUS_REPO governance`）。その先で何が governance するかは定めていない。したがって操作対象のリポジトリが `LI_PLUS_REPO` のリポジトリ自身である場合、`LI_PLUS_PROJECT_LANGUAGE` はその body 言語へ届かず、届くものは同リポジトリ自身の governance が定める。下の行は、そこで挙げる3つの面について同 governance が要求するものを述べる。同 governance を網羅したものではなく、そこに挙がっていない面がそれゆえ免除されるわけではない。直上のタイトル行と1組をなす——あちらは固定されているものをパラメータ化する誤読を抑え、こちらはこれらの行が配布されてきた当のリポジトリを操作しながら body を配布先の値で解決してしまう誤読を抑える
- その governance: 操作対象のリポジトリが `LI_PLUS_REPO` のリポジトリ自身である場合、その commit body / PR body / issue body はいずれも、同リポジトリ自身の governance 言語による文を少なくとも1つ含む。その言語を定めているのも、それを欠く実行を落とすのも `.github/workflows/liplus-ci.yml` であり、値はそこで読む。これはその1つのリポジトリの性質であって、`LI_PLUS_PROJECT_LANGUAGE` が解決した値ではなく、読んでいる workspace への要求でもない（そちらの body 言語は上の行が既に定めている）。record surface へ移さず、この面に置いたままにする——直上の一文は、それが読まれるその瞬間にこの行へ解決する
