...
The comment's prose language is the one part of the shape discretion cannot reach. The evaluator reads no rules and no contract; the only language on its surface is the prompt's own, so absent a named language it writes in that. On 2026-08-08 all three evaluators at PR #1701's brake 1 did exactly that, posting findings in the prompt's language rather than the one the contract resolves for a PR comment in that workspace. The author's answers and the parent's self-review on the same PR landed in the resolved language with nobody naming it for them: both actors auto-load the contract and resolve it themselves. That asymmetry is what scopes this to the evaluator side — a language line for the author's answers would be a second copy of what auto-load already carries, and the second copy is what drifts.
...
Nothing is named for a report that does not land on a PR — brake 2, out of this section's scope for the reason the scope paragraph gives, and the Trigger entries with no PR surface. The verdict returns into parent context there, and whatever surface eventually reaches the human is the parent's own to resolve, which it already does.
