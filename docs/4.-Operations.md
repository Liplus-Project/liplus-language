...
4. drift set を計算する。`origin/main` の docs/ ツリーと wiki 作業ツリーで内容が異なるファイルだけを
   ...
   - unclassified = `{tmpdir}/` に存在し、docs/ 所有でも Wiki 専属リストでもない名前。空でなければ
     該当を名指しして STOP しエスカレートする。参照実装はこの地点で非ゼロ終了する。停止を散文の一行に
     委ねず実行形で持つ。このガードが塞ぐのは削除ではなく**沈黙の素通り**である。手順 5 は copy のみで
     delete を持たないため、STOP を見落としても Wiki からは何も消えず、古いページが残ったまま同期完了
     として報告される。削除せず手順 5 へも進まない。docs/ 側の rename / 削除もここに出るが、リストが
     追いついていない Wiki 正本ページも同じくここに出る。エスカレーションは各名前を、source ref 上の
