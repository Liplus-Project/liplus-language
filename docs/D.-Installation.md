...
| `USER_REPOSITORY_EXECUTION_MODE` | `trigger`（人間主導）/ `semi_auto`（半自動、patchはAI直接マージ、minor / major は人間確認）/ `auto`（AI自律）。未設定ならセッション開始時にAIが聞いて設定 |
...
8. `rules/**/*.md` を読み込み（L1–L4 の常時ロード分、subdir 含む常に必須。L5 Notifications と L6 Adapter は `rules/<layer>/` を持たない予約スロット。詳細は [判断記録 d.-layer-reorg-rationale](d.-layer-reorg-rationale)）。`skills/**/SKILL.md` はトリガー時に読み込み（hookが無い環境では Codex adapter のトリガー表に従って手動読み込み）
9. 環境に応じた設定ファイルを自動生成
