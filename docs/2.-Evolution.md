...
実行後は、索引 `MEMORY.md` の先頭に `**Last consolidate run:** <YYYY-MM-DD> (<N> entries indexed)` の 1 行として記録する（`<N>` はその時点の索引行数）。各 memory ファイルへは配らない。前回実行は memory 集合につき 1 つの事実であり、同じタイムスタンプを全ファイルへ複製すれば drift するのは複製側である（`rules/model/subtractive-structural-beauty.md` Core principle (A)）。発火条件の両腕はこの 1 行から読む。経過は日付から、追記件数は現在の索引行数から `<N>` を引いて求める。行が無ければ未 consolidate と読み、トリガーは発火する。
