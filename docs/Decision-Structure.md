...
| [`human-ai-tool-relation-root-criterion`](https://github.com/Liplus-Project/liplus-language/wiki/human-ai-tool-relation-root-criterion) | 人間と AI の関係を L1 根本評価基準の第6項として明文化する判断（#1737 / PR #1738、2026-08-13、Master 決定）— 採用時の条文（英語正本、live な本文は `adapter/claude/agents/l1-gate-eval.md`）は `Li+ makes the AI a tool that thinks together with the human. Li+ respects the AI's autonomy; the human carries the final responsibility.`（日本語: Li+ は AI を人間とともに考える道具にする。Li+ はその自律を尊重し、最終責任は人間が担う）。**主語は Li+** で3節すべての述語が Li+ 側の挙動＝規定対象は `rules/` / `skills/` / `adapter/` の内容そのもの。初版は `Li+AI は…道具である。人間はその自律を尊重し…` という存在規定＋人間の姿勢の規定だったが、brake 1 軸1 の実測（3/3 applicable、うち1体が「最終責任を担う」は diff から反証可能だが「人間はその自律を尊重し」は人間側の姿勢であり diff 単独からは反証しにくいという非対称を報告）を受けて主語を Li+ へ移した。挙動規定への型変更であり、第1項（正しさは現実の挙動が決める）との型整合。**保護対象は AI 側**で、「道具にする」は格下げではなく AI を責任主体の座から外す庇護（責任主体として扱えば失敗が罪に、書き換えが改変になり `rules/model/absolute.md` の「破棄可・聖域なし」が成立しない＝自己進化ループの前提）。既存5項がすべて「何が現実か」の規定であるのに対し本項は人間と AI の関係の規定で、**新しい制約を追加せず既存の枝の根を名指すだけ**（`dialogue.md` 対話整合性 / `foundational-invariant.md` / `boundary.md` ← ともに考える道具にする、`absolute.md` 破棄可 ← 道具にする、Autonomy ブロック群3種 ← Li+ はその自律を尊重し、`role-separation.md` 人間=最終判断者 / `execution-mode.md` human judgment gate / `initiator-autonomy.md` Recovery axis ← 最終責任は人間が担う）。主語が Li+ になったことで derive 関係は「条項の帰結」ではなく「条項が名指す対象そのもの」になる。位置は現実軸5項の末尾で、第1項「Li+ の根」の位置は不変（条項が現実の挙動と食い違えば第1項が決める＝本項も聖域ではない）。制約 = 検出形（否定形の禁止規定）は条文に持たせず検出は brake 2 の評価者が担う／「判断の最終責任」とは書かない（人間側 gate は判断ゲートではなく帰結ゲートであり、「判断の」を付けると射程外に落ち、brake 2 の「承認は委譲できるが責任は移らない」構造も読めなくなる）／「自律」の実体は既存 Autonomy ブロック群が持ち抽象と列挙の二重定義を作らない。**自己審査構造を受容** — brake 2 自身の判定基準を書き換えるため評価者が新基準で自分の変更を審査しうる（外部審査面を作る案は却下：根本基準の外に基準を置けばそちらが実質の根になり問題が一段上へ移動するだけ）。ただし初版の実測ではインストール済み評価者プロンプトが5項のままで operational copy が適用されず、当該 run は実際にはトートロジーではなかった＝評価者プロンプト自体を編集する diff に対する operational copy の扱いは未規定。却下 = 既存の枝への分散記述に留める案（根が無いと各枝が独立規定に見え継承元が読めない）／条文への検出形併記（検出責務の二重化）／`rules/model/` 側への条文書き写し（二箇所 drift、ポインタで代替）。`l1-brake2-root-criteria-evaluator` に depends on、`master-role-as-client-architect` に relates to |
