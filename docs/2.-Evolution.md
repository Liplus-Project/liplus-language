...
- brake 1 のコメントスレッドの**形**は skill 側で固定してあり、親が毎回作文しない。所見のある側は full、誰も争っていない側は 1 行という非対称である。評価者コメントは、所見のある軸＝逐語引用＋`パス:行`（名指しした SHA 時点）＋なぜ欠陥か／所見の無い軸＝1 行（軸名＋その軸の語での判定＋根拠の `パス:行`。リポジトリ全体を掃く軸なら掃き出し範囲とヒット数）。著者の回答は、却下＝full（単巡打ち切りの下で誰も再反論しないため、これが親が step 7 で読む面そのものである）／受入＝1 行（何が変わったかは diff と commit に既に外部化されている）。両側で禁止するのは、親がプロンプトで渡した基準・閾値・軸文言の復唱である。所見側の逐語引用は削減対象ではない——著者は引用に当てて裁定するため、削るとソースの取り直しが指摘ごとに発生し、リテラルに当てずに断定する形（`rules/model/trigger-check-gate.md` Literal check / #1673 cluster）へ戻る。所見の無い軸で「見て clean と言った」を支えるのはポインタの**解決可能性**であり、名指しした SHA で開けること、開いて判定と合わないなら 1 回の参照で露見することがその働きである。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Report shape
