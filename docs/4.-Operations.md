...
この停止点には2回到達し、上の文言が両方の停止条件のすべてである。
...
- 2回目 = 指摘が PR コメントとして載った状態（brake 1 は評価者が直接投稿し、brake 2 だけは評価者に投稿面が無いため親が代わりに載せる）でこのサブエージェントを再開し、それが指摘を裁き、各指摘に PR 上で答え、受け入れた分をプッシュした時点。再び CI green に到達したところで委譲が終わる。何も受け入れなかった場合は新しいコミットなしで同じ点に到達する。

指摘の裁定は2回の停止のあいだで起き、再開したサブエージェントのものであって親のものではない（`rules/evolution/initiator-autonomy.md` Two-stage brake の Adjudication actor）。親は2回目の停止の後にセルフレビューしてマージする。brake の位置そのものはここに再掲しない（日本語側の解説は進化レイヤー仕様書「実行モード」節にある）。
...
3. drift set を計算する。docs/ source と wiki 作業ツリーで内容が異なるファイルだけを列挙し、その集合
   ...
   Wiki 全体になり、`rules/model/subtractive-structural-beauty.md` の Artifact deletion calibration
