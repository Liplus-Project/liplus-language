...
3. **Subagent spawn** - Select N, M, P per draft nature and spawn; where the selection puts more than one subagent in the round, spawn them in parallel. Default `N=1, M=all axes, P=1`, total invocation = 1; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   - on the brake 1 path, the resolved body language for that comment, named as the value for this run. The evaluator now writes to a surface the workspace language contract reaches and cannot resolve the value from its own context; `skills/task-subagent-prompt/SKILL.md` Delegation prompt hygiene carries the precedence, and the bound against writing a resolved value into any file
...
- **Language** resolves to the same value, by the same seams — verbatim quotes stay as the source has them. The author needs no fresh naming of it: the value was named in the delegation prompt that opened its session and the resume keeps that context (`skills/task-subagent-prompt/SKILL.md` Delegation prompt hygiene, which also covers the fallback spawn's own prompt).
