...
実行主体はマージ判断を持つエージェントである。`auto` / `semi_auto` では親、`trigger` ではサブエージェント（`skills/task-subagent-delegation/SKILL.md` のルール）。`auto` / `semi_auto` で委任されたサブエージェントはセルフレビューを実行も投稿もせず、PR open + CI green で停止して報告する。
