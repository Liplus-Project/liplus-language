...
ファイル所有境界（2026-04-26 以降）：

- **docs/ 所有ファイル**（大文字 + 数字 prefix + `Home.md` + `_Footer.md`）: docs/ が正本、Wiki は同期先。同期後は byte 単位で一致する
- **Wiki 専属ファイル**（小文字 prefix `[a-z].-*.md` の判断記録 + `_Sidebar.md` 等の Wiki 専用ナビ）: Wiki が所有、docs/ に counterpart を持たない、同期は preserve する
- 例外として `docs/a.-Decision-Log.md` は **docs/ と Wiki の両方に存在**する（docs/ 側は `adapter/claude/hooks/on-session-start.sh` が cold-start synthesis 素材として head を emit、Wiki 側はナビでの可視化）。docs/ が source なので sync 対象に含める

push 前検証（手順 5/6 のコミット前に必須）：

- `git -C {tmpdir} status --short` を実行し、削除（`D`）と更新（`M`）に docs/ 所有パスのみが現れることを確認する
- Wiki 専属ファイル（小文字非 `a` prefix `[b-z].-*.md` または `_Sidebar.md`）に `D` / `M` が出た場合は **STOP** して人間にエスカレートする。selective wipe パターンの逸脱は再発する故障モードであり、この信号で wiki に push してはならない
- Sidebar 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/_Sidebar.md` が navigable entry を網羅的に参照しているか検証する。期待 slug 集合を `{tmpdir}` ファイルシステムから構築する：
  ...
  - `{tmpdir}/[0-9]*.md` のすべて
  - `{tmpdir}/[a-z].-*.md` のすべて（`a.-*` と Wiki 専属の `[b-z].-*` 判断記録）
...
- 初期 docs/ に `Home.md` / `_Footer.md` / 正規の大文字 + 数字 prefix ファイル（`docs/[A-Z].-*.md`、`docs/[0-9].-*.md`）を配置
- `_Sidebar.md` は wiki repo 側に直接 push（docs/ 経由ではない）
- 判断記録（`[b-z].-*.md`）は作成時点から Wiki 専属。docs/ には置かない
...
3. 選択 wipe: docs/ 所有ファイルのみ Wiki から削除、Wiki 専属エントリは残す:
   ...
   ※ `[b-z].-*.md` と `_Sidebar.md` は明示的に対象外
4. docs/ をコピー: cp docs/*.md {tmpdir}/
   ※ 移管後の docs/ には `b.-` 以降の判断記録は無いので、この cp は wiki-only エントリを再導入しない
