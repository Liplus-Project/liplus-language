...
(→ `skills/operations-on-release/SKILL.md` 権威ソース。本節はその日本語ミラー)
...
**Application-moment trigger（判断形成の瞬間で発動）：** バージョン分類（patch / minor / major）を任意の artifact に書く直前に、本節を literal 再読する。再発失敗は minor の "large" 修飾子省略 — observable な変更を minor と誤分類するが scope は incremental（つまり patch）というケース。詳細は `skills/model-trigger-check-gate-actions/SKILL.md` の Trigger moments を参照。
...
- Sidebar 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/_Sidebar.md` が navigable entry を網羅的に参照しているか検証する。期待 slug 集合を `{tmpdir}` ファイルシステムから構築する：
  ...
  `{tmpdir}/_Sidebar.md` から `](<slug>)` リンク先を抽出し、`期待 − 参照` が空でなければ **STOP** して欠落 slug を名指しで人間にエスカレートする。この信号で wiki に push してはならない。sidebar drift は entry を追加した PR がナビを保守しなかったということであり、release sync は黙って自動修正する層ではない。
...
- Cross-reference 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/*.md` 内のすべての wiki 内部リンクが実在 entry を指していることを検証する。解決集合を構築する：
  - 実在 slug = `Home` + `{tmpdir}/[A-Z]*.md` + `{tmpdir}/[0-9]*.md` + `{tmpdir}/[a-z]*.md`（すべて拡張子なし）
  - 抽出 slug = `{tmpdir}/*.md` 本文内のすべての `](<x>)` 出現箇所のうち、`<x>` が `://` を含まず、`#` で始まらず、`/` を含まないもの（wiki 内部リンク）。`<x>` に `#section` フラグメントがあれば剥がしてから resolution する
...
3. 選択 wipe: docs/ 所有ファイルのみ Wiki から削除、Wiki 専属エントリは残す:
   shopt -s nullglob
   for f in {tmpdir}/[A-Z]*.md {tmpdir}/[0-9]*.md {tmpdir}/Home.md {tmpdir}/_Footer.md; do
     [ -e "$f" ] && rm -f "$f"
   done
   ※ `[a-z]*.md`（判断構造）と `_Sidebar.md` は明示的に対象外。`Decision-Structure.md`（大文字 `D` 始まり）は `[A-Z]*.md` でカバーされる通常の docs/ 所有ファイル
4. docs/ をコピー: cp docs/*.md {tmpdir}/
   ※ docs/ は大文字 + 数字 prefix + `Home.md` + `_Footer.md` + `Decision-Structure.md` のみ。判断構造 entry は wiki 専属なので、この cp は wiki-only エントリを再導入しない
...
Wiki 同期はリリース手順の一部であり、後続タスクに分離しない。

#### リリース後のマイルストーン削除

(→ `skills/operations-on-release/SKILL.md` の `## Post-release milestone delete` + `skills/operations-on-milestone/SKILL.md`)

Wiki 同期が成功した後、当該リリースのマイルストーンを削除する。これはリリースフロー完了の必須ステップであり、後続タスクに分離しない。

```
gh api -X DELETE repos/{owner}/{repo}/milestones/{milestone_number}
```

DELETE は open milestone に対して直接効く（close ステップ不要、実機検証 2026-04-21）。根拠: GitHub milestone UI にリリース後の情報保持価値はなく、監査証跡はリリースノート + PR / commit history に残る。省略すると closed 相当の停滞マイルストーンが蓄積する（dogfood 2026-04-21: v1.14.1 / v1.14.2 / v1.15.0 / v1.15.1 が open のまま 4 件蓄積していた）。
