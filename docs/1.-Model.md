...
本件は Body States Behavior の適用結果を**外部 probe で測った**最初の例である。#1848 の段2構成で A 腕 = full 版 / B 腕 = 蒸留版を各 3 本走らせ、`description` を 1 バイト違わず同一に保ったうえで 5 問全問一致・差ゼロを得た。ただし probe は `Three roles` 節・Web 側 baseline reminder・Tier/Stage 説明段落に触れていないため、結論は「この 5 問の範囲で同じ振る舞いをした」までである。本文に `Judgment record:` ポインタは置いていない——足せば merge される本文が測定された本文でなくなるため、ポインタ追加は独立の作業単位として分けた。
