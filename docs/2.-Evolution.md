...
進化ループは AI 主導で全周を回す。人間ゲートはリリース / 不可逆系の軸にのみ残る。
...
同じ原因パターンが繰り返された場合、`Evolution_Initiator_Autonomy` の initiator 経路で spec 改善を起票する。自己進化 PR は AI 主導で回し、brake 1（`skills/parallel-subagent-eval`、N>=3）を必ず通す。L1 Model Layer の変更は加えて brake 2（`adapter/claude/agents/l1-gate-eval.md` 評価者の PASS）を要する。変更ごとの人間 go-sign は不要で、人間ゲートはリリース / 不可逆系の軸にのみ残る。
...
- 人間ゲートはリリース / 不可逆系の軸にのみ残る（人間 = 最終審判者は別軸）
...
現行は AI 主導で全周を回す。各段階を AI 単独で回し、人間ゲートはリリース / 不可逆系の軸にのみ残す（L1 更新のゲートは根本基準評価者 brake 2、上記「brake 2 との関係」参照）。人間への確認を段階ごとに差し込むと閉路が回らなくなるため、自律可能な段階を自律的に進める姿勢を既定とする。
