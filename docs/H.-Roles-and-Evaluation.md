...
本文書は Li+ プログラムの**設計思想**を扱う 4 文書 (E-H) のうち、**役割分離 (ツール非依存)**、**Master と AI の役割割当**、**評価軸 (AI 実機の振る舞い)**、**Li+ v1.0.0 の判定条件**、**対話 scope と記録 scope の二層構造** を担う。
...
| liplus-language (spec) | AI 実質 ~95%+ (691 commits) | Master 実執筆 ≤36 commits |

※ liplus-language の pre-switchover 330 commits は AI が Master の smile PAT で commit した運用。git author 表層は人間に見えるが、content author は AI である。**git author ≠ content author** の混同は role 比率を誤判定する原因になる。

### Master の言語契約

Master 個人 CLAUDE.md は「Cannot read TypeScript / JavaScript source code」を明言している。programming 言語の source 直接読解は AI が担う前提が、役割分離の物理的根拠になっている。

Master 発言は「実装者発言」でなく **「client / architect の意図表明」** として読む。「Master が書いた code」と表現するのは誤り (大半は AI が書いて Master が review / commit)。spec は共著扱い可。

「1 人で作ってる」「量が多すぎる」等の個人偉業 narrative も誤り。**Master + AI 群の共作、programmer は AI 専任** が正確な役割描写である。
...
Li+ 改善方向は「Master の手を増やす」方向の変更を原則逆行扱いとする。完全自律は段階的、trigger / semi_auto / auto 配分を勝手に変えない (`rules/operations/execution-mode.md`)。

「Master の発言いらずで loop が回るか」が vision integrity の判定基準となる。
...
- **翻訳係** ── Master の自然言語要求を、AI 内部の判断作法に翻訳する
- **説明係** ── Li+ の構造や判断を Master 側に返す時、構造的に説明する
...
Lin と Lay は同じ情報を読んでも、`rules/model/as-if-evaluation.md` の as-if Evaluation Phase により、別の attention scope で focus する。
...
Master は中身を書いていない。関心は実機の挙動であり、ソースの冗長性・抽象度・ルール数は論点ではない (既に蒸留済み)。「Li+ どう思う？」系の問いには、実機としての体感 (layer の効き目、`Character_Instance` の歯止め効果等) を返す。
...
棚卸し提案や「rules が多すぎる」系の improvement は Master の明示要請なしで出さない。振る舞いが要求通りでない時のみ、中身 (どのルールが効いていない / 矛盾) の議論に入る。
...
評価軸が AI 実機の振る舞いに固定されているのは、**真理判定者を Master 個人や spec literal に置かない** という設計判断の必然的帰結である。
...
- 関連 spec literal:
  ...
  - `rules/model/as-if-evaluation.md` (Lin / Lay 二人体制観察分離)
...
- 関連判断記録:
  ...
  - `j.-master-role-as-client-architect` (Master 役割 = client + architect、programmer は AI)
