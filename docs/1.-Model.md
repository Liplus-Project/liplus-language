...
評価者が判定するのは、**変更の内容**が L1 の変更としてふさわしいかどうかであって、**その変更を入れるべき根拠がどれだけ積まれているか**ではない。inline で渡される stated reason についても、証拠としての充足度は採点対象ではない。
...
その変更自身が負う現実の挙動の観測はマージの後ろにある（`rules/operations/operations.md` Post-L1-Merge Runtime Observation、`rules/evolution/memory-entry-format.md` Self-Evolution Observation Format）。証拠を事前条件として要求する読みではこれらが起動し得ず、ブロック自身が存在を妨げている観測を要求する形になる。
