...
- `docs/Decision-Log.md` の先頭（section key: `decision_log_head`）
...
- `jq` コマンドが PATH にない（stdin の matcher 解析は sed regex fallback を持つが、state 読み書きは jq 必須）
