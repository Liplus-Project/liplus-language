...
3. **Subagent spawn** - Select N, M, P per draft nature and spawn; where the selection puts more than one subagent in the round, spawn them in parallel. Default `N=1, M=all axes, P=1`, total invocation = 1; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   The spawn splits on evaluator kind, the same split step 2 above reads. A judge-type evaluator — one that reads the draft — spawns at the effort floor (see Constraint: Effort floor); on Codex it still spawns under the `brake-evaluator` agent definition, but on Claude Code no per-role definition file carries the role any more (`adapter/claude/agents/` was reorganized by effort, not by role — #1972). Claude Code spawn: `subagent_type: medium` (or a higher effort-named agent), with the brake 1 evaluator role literal (Constraint: Effort floor) injected into the prompt verbatim. A probe-type evaluator — one whose own bare behavior under the applied draft is what the round reads — spawns with no definition file and inherits the parent session's effort, so raise the parent session's effort to the floor before spawning one, and leave it there for that round. Nothing enforces that raise: effort is not a spawn-call parameter, so a probe-type round the parent does not raise its own effort for runs at whatever the session was set to, and `rules/model/subtractive-structural-beauty.md` puts a step of that shape on the replace-with-a-structure side.
...
- **Effort floor = `medium`**: no host passes effort at the spawn call, so an evaluator spawned with no definition takes whatever the parent session is set to, and a definition file naming an `effort` is the only surface that fixes it; where a round spawns a probe-type evaluator, the floor is reached by raising the parent session instead (Procedure step 3). On Codex a judge-type evaluator still spawns under `brake-evaluator` (`adapter/codex/agents/brake-evaluator.toml`), whose `effort` is that floor. On Claude Code `adapter/claude/agents/` carries no per-role definition (#1972: reorganized by effort, not by role); the floor is reached by naming an effort-floor-or-above agent at the spawn call instead — `subagent_type: medium` — and the role that used to live in `brake-evaluator.md`'s body is carried by the prompt as the literal below, copied verbatim and not re-composed per spawn:
...
Subagent spawn goes through the host's Agent tool (Claude Code: `Agent` tool; Codex: equivalent mechanism). Parallel execution = multiple Agent tool calls in a single message, and every call explicitly names a `subagent_type` — omission is not a shorthand for the built-in general-purpose agent, it is named as such (`skills/task-subagent-spawn/SKILL.md`). A probe-type evaluator spawns as the host's built-in general-purpose agent, with no Li+ agent definition file: its bare behavior is what such a round reads, and a definition body would replace the system prompt it reads (`skills/task-subagent-spawn/SKILL.md`). A judge-type evaluator reads the draft rather than being read, so on Codex it spawns under the `brake-evaluator` definition, whose body carries the role and nothing of the evaluation; on Claude Code it spawns under an effort-named agent (`subagent_type: medium` or above) with the same role carried by the prompt instead (Constraint: Effort floor). The implementation delegate's role reaches neither: on Claude Code it is carried by `skills/task-subagent-prompt/SKILL.md` Role literal: implementation delegate, injected only into delegation prompts under `skills/task-subagent-delegation/SKILL.md`.
