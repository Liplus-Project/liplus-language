...
3. **Subagent spawn** - Select N, M, P per draft nature and spawn; where the selection puts more than one subagent in the round, spawn them in parallel. Default `N=1, M=all axes, P=1`, total invocation = 1; switch to the M=1 exception pattern when the echo-bias condition in Constraint: Subagent prompt must be self-contained holds, or to P>1 when premise variation is needed (see Design Dimensions). Every spawn explicitly sets the Agent tool `model` parameter at or above the sonnet-class floor (see Constraint: Model floor).
   ...
   The spawn splits on host and evaluator kind, the same split step 2 above reads. On Claude Code, a judge-type evaluator — one that reads the draft — spawns under `subagent_type: brake-evaluator`, which is where its effort floor stands; a probe-type evaluator — one whose own bare behavior under the applied draft is what the round reads — spawns with no definition file and inherits the parent session's effort, so raise the parent session's effort to the floor before spawning one and leave it there for that round. Nothing enforces that Claude raise: effort is not a spawn-call parameter there, so a probe-type round the parent does not raise its own effort for runs at whatever the session was set to, and `rules/model/subtractive-structural-beauty.md` puts a step of that shape on the replace-with-a-structure side. On Codex, both kinds spawn with no agent definition file and explicitly pass `reasoning_effort="medium"`; the self-contained prompt carries the judge role when needed, while the probe keeps its bare behavior as the observation target (see Constraint: Effort floor).
...
- **Effort floor = `medium`, resolved independently from the model floor**: Claude Code judge-type evaluators spawn under `brake-evaluator` (`adapter/claude/agents/brake-evaluator.md`), whose `effort` is that floor. Claude probe-type evaluators take no definition, so the floor is reached by raising the parent session instead (Procedure step 3). Codex judge-type and probe-type evaluators take no agent definition and explicitly pass `reasoning_effort="medium"` on every spawn; `adapter/codex/agents/*.toml` must not set `model_reasoning_effort`, whose precedence would override that resolved per-launch value. The Codex value must be one the selected model exposes; do not guess a fallback for an unsupported value. The explicit `model` parameter continues to enforce the separate model floor above, and the no-write requirement stays on the prompt literal below
...
Subagent spawn goes through the host's Agent tool (Claude Code: `Agent` tool; Codex: equivalent mechanism). Parallel execution = multiple Agent tool calls in a single message. A probe-type evaluator spawns as the host's built-in general-purpose agent, with no Li+ agent definition file: its bare behavior is what such a round reads, and a definition body would replace the system prompt it reads (`skills/task-subagent-spawn/SKILL.md`). On Claude Code, a judge-type evaluator reads the draft rather than being read, so it spawns under the `brake-evaluator` definition, whose body carries the role and nothing of the evaluation. On Codex, the judge-type evaluator also uses the built-in agent; the self-contained prompt carries its role and the spawn call carries `reasoning_effort`. The implementation delegate's definition reaches neither.
