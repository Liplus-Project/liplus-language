...
本文書は Li+ プログラムのオペレーションレイヤー（`rules/operations/*.md` + `skills/operations-on-*/SKILL.md`）の仕様を定義する。
...
Issue Format、Issue Maturity、Sub-issue Rules、Milestone Rules は個別 skill（`skills/operations-on-issue-format`、`skills/operations-on-issue-maturity`、`skills/operations-on-sub-issue`、`skills/operations-on-milestone`）に配置されている。
...
メインエージェントが PR diff を issue 要件と照合してレビューする（`skills/task-pr-review-judgment/SKILL.md` 参照）。
...
権威ソース: `rules/operations/release-version.md`。本 docs はその日本語ミラーである。
