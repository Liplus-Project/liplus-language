...
- body 行が解決するのは host workspace に対してのみである。操作対象のリポジトリが `LI_PLUS_REPO` のリポジトリ自身である場合、body 言語は同リポジトリ自身の governance が解決し、`LI_PLUS_PROJECT_LANGUAGE` はそこへ届かない。その到達範囲の正本は `Workspace_Language_Contract`（`adapter/claude/CLAUDE.md` / `adapter/codex/AGENTS.md`）の `They do not change LI_PLUS_REPO governance` であり、この行はそこを指すポインタであって契約の写しを持たない。直上のタイトル行と1組をなす——あちらは固定されているものをパラメータ化する誤読を抑え、こちらはこれらの行が配布されてきた当のリポジトリを操作しながら body を配布先の値で解決してしまう誤読を抑える
