...
- **測定の射程は plan の必須フィールドとして走行記録に載る**。plan JSON は `scope` オブジェクト——`probes_total` 問中 `probes_measured` 問を測ったか、選定基準 `selection`、測っていない範囲 `unmeasured`——を持たなければ拒否され、その値はそのまま走行記録へ運ばれる。1 走行は 1 probe なので、これは当該走行が属する周の射程である。値は自己申告であり検証しない（正しさの検証は受け入れて手放す）。結果は射程と併せて読む——「蒸留が何を壊すか」の予測で選んだ周が green なら、言えるのは「予期した壊れ方は起きなかった」であって「何も変わらなかった」ではない。射程を人が書き添える手順にすると、書き忘れても何も落ちず部分測定が全面測定として読まれるため、構造に置いた（`rules/model/subtractive-structural-beauty.md` の手続き→構造置換。30 問中 8 問を測った PR #2012、5 問の #1851 と同じ欠落が 2 回起きた。#2013）
