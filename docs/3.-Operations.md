...
長時間セッションでの注意劣化対策として、pre-push フェーズ（on_branch, on_commit）の作業をサブエージェントに委任できる。サブエージェントは毎回新鮮なコンテキストで Li+operations.md を読むため、注意の薄まりが発生しない。

post-push フェーズ（on_pr, on_ci, on_review, on_merge, on_release）は remote state 操作が本体のため委任しない。
