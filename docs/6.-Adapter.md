...
JSON 処理：stdin payload の解析と `hookSpecificOutput` の組み立ては `node -e` で行う。node は Claude Code 自身が依存するランタイムのため安全な前提（`on-session-start.sh` と同方針、#1519 / #1540）。#1540 以前は外部 `jq` バイナリに依存しており、jq 未導入ホスト（Windows / macOS / 多くの Linux ディストリビューションで既定では同梱されない）では tool 名の解析が空となり、hook が**フォールバックも痕跡も無い無言の no-op** に陥っていた。なお本 hook 内の `gh api --jq` は gh バイナリ内蔵の jq 式エンジンであり、外部依存ではないため置換対象外。
