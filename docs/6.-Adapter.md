...
- `docs/a.-Decision-Log.md` の先頭（section key: `decision_log_head`）
...
fail-safe 動作（startup matcher 時）：以下のいずれかが発生した場合は全 section を full emit し、instruction footer に理由を Master observable な形で記載する：
