...
- Cross-reference integrity assertion (post-step 4, pre-step 5): verify every wiki-internal markdown link target in `{tmpdir}/*.md` resolves to an existing file. Before extraction, remove code notation from the body: link-like notation inside code, or inside an HTML comment, is documentation about the notation or invisible markup, not a real link. Remove block-level code notation first, then inline code spans:
  ...
  - HTML `<code>` / `<pre>` elements and HTML comments (`<!-- ... -->`).
  Then remove inline code spans: a run of N backtick characters, closed by the next run of exactly N backticks; if no closing run of exactly N backticks exists, the opening run is not a code span and is left as literal text.
  Removing block-level notation first prevents a fence's own backtick run from being mistaken for an inline-span delimiter and swallowing unrelated text. Build the resolution set:
