...
- Cross-reference 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/*.md` 内のすべての wiki 内部リンクが実在 entry を指していることを検証する。抽出前に、本文からコード記法を除去する。コード内、または HTML コメント内のリンク風記法は記法そのものの説明か、描画されない markup であり、実リンクではない。次の順序で除去する。各パスは本文全体に適用してから次のパスに進む：HTML コメント、次に HTML 要素、次にフェンスドコードブロック、次にインデントコードブロック、最後にインラインコードスパン。
  ...
  コード記法除外（#1547）：除去なしだとアサーション自身の仕様文が自己一致してしまう。本ファイルと `skills/operations-on-release/SKILL.md` はパターンをインラインコードスパン内の `](<x>)` 記法で説明しており、wiki entry `wiki-sync-sidebar-integrity-check.md` も同様である。これにより全リリースで偽陽性 STOP が 3 件発生していた（2026-07-26、v1.19.11 / build-2026-07-26.3 で実測）。コード記法を除去した結果、実リンク 66 件はすべて解決し、真の未解決リンクはゼロだった。
