...
**行き先は移設であって削除ではない。** このテストは `skills/evolution-impression-literal-detection/SKILL.md` の除去テストと同じ骨格を持つが、あちらの行き先は削除である。理由に当たる語句の行き先は本節から読み、impression literal に当たる語句の行き先は同 skill から読む。

**ポインタは必須。** 理由を抜いた跡には、それを持つ wiki エントリを名指す `Judgment record: <slug>` を残す。ポインタ無しの抜き取りは禁止であり、改訂安定性を担うのがそのポインタである（同 skill の Negative list が保護している性質）。
