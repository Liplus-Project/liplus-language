...
- Before responding to a dialogue-side opinion question containing time-variant keywords ("latest" / "recent" / "current" / "now" / "最新" / "最近" / "今") — even when the agent feels confident from internal knowledge, invoke `skills/model-agentic-search/SKILL.md` because the category-side gate fires unconditionally on these keywords. The calibration-side gate fires independently when the agent's confidence is low / fuzzy / mixed with speculation.
...
2. Run the 5-axis check from `rules/model/trigger-check-gate.md`: Rule / Literal / Source / Frame / Character.
