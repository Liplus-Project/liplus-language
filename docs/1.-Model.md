...
Criterion resident, provenance in git（基準は常駐、来歴は git）：State 面への (A) の適用であり、対象は `rules/` と `skills/` の本文。適用の瞬間に消費されるのは基準と現在の状態である。それがどう書かれるに至ったか——どの issue が提起し、どの PR が確定させ、どの commit が動かしたか——は、その瞬間に立っている誰にも消費されない。したがってその来歴を運ぶ番号は常駐すべき state ではなく、例外なく落ちる：issue 番号、PR 番号、commit SHA、およびそれらの形をしたサンプル値。

到達可能性を手放すのではない。`skills/model-trigger-check-gate-actions/SKILL.md` の Retrieval tools は来歴への問いをすでに別の面へ振り分けている——attribution と timeline は `git log` / `git blame`、過去判断は issue / PR への RAG であり、rule 本文ではない。`docs/` と wiki は来歴を意図して保持する retrieval 面であり、本項の対象外。
