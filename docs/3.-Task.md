...
この2行は `rules/operations/operations.md` の `## Operations Rules` が持つ軸にそのまま乗る。タイトル軸、および操作対象のリポジトリが `LI_PLUS_REPO` のリポジトリ自身である場合に本文言語を同リポジトリ自身の governance が解決する carve-out も同節が正本であり、ここには写さない。
