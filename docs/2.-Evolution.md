...
現行は部分自動化だが、AI が単独で回せる段階は AI 単独で回す。人間の承認が必要な段階（リリース）だけを人間に渡す（L1 更新のゲートは根本基準評価者 brake 2、上記「brake 2 との関係」参照）。人間への確認を段階ごとに差し込むと閉路が回らなくなるため、自律可能な段階を自律的に進める姿勢を既定とする。
