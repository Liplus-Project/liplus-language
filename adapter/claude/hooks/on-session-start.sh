...
#   LI_PLUS_UPDATE_STATUS=unnecessary     -> AI skips Li+update walkthrough; Li+config spot read for value lookup is permitted
...
if [ "${#UPDATE_REASONS[@]}" -eq 0 ]; then
  ...
  printf 'Sentinel-skip applies: AI skips Li+update.md re-execution this session. Li+config.md on-demand spot read for value lookup is permitted.\n'
