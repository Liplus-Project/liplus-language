...
3. **output-styles/ ファイル：** `rules/model/character_Instance.md` の body を読み出し、output-styles 形式の frontmatter（`name: character_Instance` / `description`）に書き換えて `.claude/output-styles/character_Instance.md` に install する。Claude Code 標準の output-styles 機構が session 開始時に system prompt 冒頭へ rendering し常駐させる。settings.json の `"outputStyle": "character_Instance"` で active 化。初回のみ生成し既存ファイルは上書きしない（ユーザーカスタマイズ可能）。旧 rules slot（`.claude/rules/model/character_Instance.md`）からの一回限り migration を bootstrap が担う（詳細は Li+bootstrap.md Phase 4c.2）。
