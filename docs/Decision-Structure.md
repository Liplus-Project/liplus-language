...
| [`parallel-subagent-eval-model-floor`](https://github.com/Liplus-Project/liplus-language/wiki/parallel-subagent-eval-model-floor) | brake 1 評価者のモデル床判断（#1482、#1532 で opus 級から sonnet 級へ更新）— per-call `model` 明示指定で sonnet 級床を固定（暗黙親継承禁止 / doubt→sonnet fallback、`haiku` は床未満として禁止）、custom-agent frontmatter ピン留めは judge/probe 混在ゆえ不採用（per-call は context / identity 面を変えない非対称が根拠） |
