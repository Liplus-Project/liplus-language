...
書き込み前の永続化ティアリング判定（hard gate）、既存のメンテナンスルール、明示的な「書くな」指示の scope 限定は、いずれも `adapter/claude/CLAUDE.md` の `Memory_Write_Autonomy` が正本である。書き込む前にそちらを読むこと。ここには再掲しない——過去に後ろ二つだけを写した結果、後から正本にだけ追加された hard gate が写しに届かず、`rules/evolution/memory-entry-format.md` Trigger point が断とうとしている「memory に永続情報が滞留する」経路がこの面からは素通りに見えていた。二つ目のコピーが drift する。
