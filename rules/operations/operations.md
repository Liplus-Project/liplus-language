...
The body lines resolve against the host workspace only. When the repository being operated on is the repository at `LI_PLUS_REPO` itself, its own governance resolves the body language and `LI_PLUS_PROJECT_LANGUAGE` does not reach it. Canonical for that reach = `Workspace_Language_Contract` (`adapter/claude/CLAUDE.md` / `adapter/codex/AGENTS.md`), `They do not change LI_PLUS_REPO governance`; this line is a pointer to it and holds no second copy of the contract. Pairs with the title line above — that one states against parameterizing what is fixed, this one against resolving a body from the host workspace's value while operating on the repository these lines were distributed from.
