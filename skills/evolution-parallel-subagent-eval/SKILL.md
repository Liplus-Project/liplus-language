...
description: Invoke when verifying a Li+ rules/* or skills/* edit before commit/merge, or when N=1 self-check on an edit feels positive — parallel subagent eval (min 3, different axes) catches introspection-gap-driven overconfidence.
layer: L2-evolution
---
...
3. **3 軸並列 spawn** — 最低 3 subagent を異なる評価軸で同時 spawn。prompt は self-contained (parent context を持ち込ませない)
...
6. **judgment** — spec 変更継続 / 修正案を反映して再 spawn / 中止 を選択
