...
上表が決めるのはどの tier に当たるかであり、`in-progress` 遷移が何で構成されるかではない。その遷移（ラベル + issue assignee、同一モーメント）は `skills/task-subagent-state-labels/SKILL.md` Work start が正本である（タスクレイヤー仕様書「actor 軸（issue assignee）」）。
...
ブランチ作成は assignee 手順を持たない。actor 軸は `in-progress` 遷移に束ねられており、ブランチ作成には束ねられていない（タスクレイヤー仕様書「actor 軸（issue assignee）」／ `skills/task-subagent-state-labels/SKILL.md` Actor axis）。
...
- **docs/ 所有 = 大文字 + 数字 prefix の `*.md`、`Home.md`、`_Footer.md`**。docs/ が正本で、同期後は Wiki 側と byte 単位で一致する。`docs/Decision-Structure.md` は index する entry がすべて Wiki 専属であっても docs/ 所有側に置く。`adapter/claude/hooks/on-session-start.sh` が cold-start 素材として docs/ 側を読むため、Wiki 側に寄せて docs/ 側を畳むことはできない
- **Wiki 専属 = 小文字 kebab-case `[a-z]*.md` の判断構造 entry、`_Sidebar.md`、その他 Wiki 専用ナビ**。docs/ に counterpart を持たないため、「docs/ に無い = Wiki から削除」と読む手順はこれらを消す。同期は preserve する。entry は順序 prefix を持たないので、順序はファイル名ではなく `docs/Decision-Structure.md` と `_Sidebar.md` で明示する
...
- **`git -C {tmpdir} status --short` の `D` / `M` が docs/ 所有パスのみであることを確認する。** Wiki 専属ファイル（`[a-z]*.md` または `_Sidebar.md`）に `D` / `M` が出ていれば、手順 4 のファイル集合が所有境界から逸脱している。この逸脱が本手順の再発する故障モード
- **Sidebar 整合性：`{tmpdir}/_Sidebar.md` が navigable entry を網羅参照しているか検証する。** 期待 slug 集合は docs/ ローカルでも index 文書でもなく `{tmpdir}` ファイルシステムから構築する（両所有系統が Wiki 上に存在する形で数えるため）：
  - `Home`
  - `{tmpdir}/[A-Z]*.md` と `{tmpdir}/[0-9]*.md` のすべて（docs/ 所有。slug は拡張子を除いたファイル名）
  - `{tmpdir}/[a-z]*.md` のすべて（Wiki 専属 kebab-case entry）
...
- **Cross-reference 整合性：`{tmpdir}/*.md` 内のすべての Wiki 内部リンクが実在 entry を指すか検証する。** 解決集合を構築する：
  - 実在 target = `{tmpdir}/[A-Z]*.md` + `{tmpdir}/[0-9]*.md` + `{tmpdir}/[a-z]*.md` の各実在ファイルについて、拡張子なし slug と `.md` 付き完全ファイル名の両方（`Home` と `Home.md` を含む）。`.md` 付き target は、この列挙で対応する Markdown ファイルが実在する場合だけ解決集合へ入る
...
- 初期 `docs/` に `Home.md`、`_Footer.md`、正規の大文字 + 数字 prefix ファイル（`docs/[A-Z]*.md`、`docs/[0-9]*.md`）を配置する。`docs/Decision-Structure.md` も含める
...
3. drift set を計算する。docs/ source と wiki 作業ツリーで内容が異なるファイルだけを列挙し、その集合
   ...
   - to_copy = docs/ 側に存在し wiki 側と内容が異なる docs/ 所有ファイル（新規 + 内容変更。解決 glob:
     `docs/[A-Z]*.md` / `docs/[0-9]*.md` / `docs/Home.md` / `docs/_Footer.md`）
   ...
   後で CR が再混入しても行末が drift にならないようにする防御）。上記 glob は小文字 kebab-case や
   `_Sidebar.md` に届かない。これらは Wiki 専属であり to_copy / to_delete に入れてはならない。
...
4. drift set を per-file 操作で適用する:
   ...
   to_copy・to_delete とも空 = drift なし。commit / push を skip して手順 8（後片付け）へ進み、
   no-op を報告する。
