...
ここで外部ツールを名指ししないのは意図的である。同梱していない skill だけを経路として書けば、その skill を持たないホストでは手順がそこで止まり、止まりに出会った agent が他者の読めない即興の pass を入れる。5 項は Li+ 自前の discipline を順に当てたものであり、Li+ が走る場所であれば必ず走る。
