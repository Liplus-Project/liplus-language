...
- brake 1 の **effort の床は `medium` であり、judge 型の定義ファイル（`brake-evaluator`）がそれを置く**。モデル床がモデル軸しか守っていなかったことが実測で出ている——effort は spawn call の引数に存在せず、定義ファイルを持たない評価役は親セッションの値をそのまま継承する。2026-09-13 の実測では本 workspace の 2 セッション全件でほぼ `low` であり、出所は `.claude/settings.local.json` の `effortLevel` であった。値は暫定であり（Master 判断、2026-09-13）、`low` の外し方——正解の literal に到達しながら「保護すべき rationale」と判断して finding から外す、探索ではなく境界判断が保護側へ寄る形——に対して最小の変更で当たる位置として 1 段だけ上げた。`medium` / `high` の効き方は未測定であり、未測定の 2 段上げは根拠を持たない。定義ファイルを持つのは judge 型だけである。judge 型は draft を読む側であって読まれる側ではないため、置き換わる観測対象をそもそも持たない。probe 型は #1482 の判断のまま定義ファイルを持たず、床へは親セッション自身の effort を上げることで届く（構造ではなく手続きであり、親が取らなければ効かない）。定義ファイルには `model` も `tools` も書かない——前者は per-call の sonnet 級床をそのまま残すため、後者は上のツール権限を絞らない判断と同じ理由である。正本は `skills/evolution-parallel-agent-eval/SKILL.md` Constraint: Effort floor と Procedure step 3（#1968）
