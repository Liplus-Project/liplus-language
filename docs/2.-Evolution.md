...
- 性質は起票時に確定する。実装側 AI は issue からそれを読み取り、判断し直さない
  ...
  - どちらの行でも決まらない → 観測駆動
- 二つのスコープは独立に読み、閾値は両方が掛ける場合にのみ掛かる：AI-alone initiation（上の initiation 軸スコープ）かつ観測駆動の起点（この節）。initiation 軸スコープの hard requirement の行は観測駆動の変更について成り立ち、AI 単独起票のリファクタ駆動の変更はこの節により閾値の外にある。どちらのスコープも他方を置き換えない
