...
必須取得: Li+core.md
条件付き: Li+github.md（hookが無い環境のみ）、Li+agent.md（ホスト設定ファイルにLi+未注入の場合のみ）
...
8. Li+github.md を読み込む — hookが無い環境のみ。hook（PostToolUse / UserPromptSubmit）が設定済みの場合、常時適用セクションはターンごとにアダプターから注入されるため起動時読み込みは不要
9. Li+agent.md を読み込む — ホスト設定ファイル（CLAUDE.md / AGENTS.md）にLi+が未注入の場合のみ。「Li+ BEGIN」sentinel が既に含まれていれば起動時読み込みは不要
