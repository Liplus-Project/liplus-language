...
**起点** = 変更された規則を実行時文脈に載せた最初のセッションであり、~5 分の予算はそのセッションの内側で使う。マージしたセッションがそれを載せている場合（Li+ ソースを `main` で走らせている workspace）は、マージ時に下の観測セットをそのまま走らせる。載せていない場合（タグに同期した workspace）は、マージしたセッションは観測せず繰り越す —— その PR の `memory/self-evolution-observation.md` エントリの `notes` に繰り越しを記録し（エントリが無い PR ではマージ済み PR へのコメントとして記録する。`rules/evolution/memory-entry-format.md` Self-Evolution Observation Format）、規則を載せた最初のセッションで観測を取って、繰り越しを記録した場所に結果を追記する。
...
クローズされた issue の本文に PR 単体では満たせない完了条件（`rules/operations/main-agent-procedures.md` の `## Issue format`）があり、その条件が未達のときは、issue を reopen し、条件が満たされるまで開いたままにする。`auto` / `semi_auto` では親がマージ直後に reopen する。`trigger` ではマージの瞬間に立つ agent が居ないため、マージを最初に観測したターンでメインエージェントが reopen する。
