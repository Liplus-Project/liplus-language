...
| # | 場所 | 誤った claim / 推論 | 実態 |
|---|------|-------------|------|
| 1 | `skills/on-pr-creation/SKILL.md`（旧 `operations/Li+github.md` [PR Creation]） | 「`Refs` triggers GitHub auto-close on merge」 | GitHub の close キーワードは `close/closes/closed/fix/fixes/fixed/resolve/resolves/resolved` のみ。`Refs` は close キーワードではない。PR #1066 merge で sub-issue 9 件が OPEN 残存して顕在化 |
| 2 | `skills/research-strategy/SKILL.md`（旧 `task/Li+issues.md` Research Strategy） | 「github-rag-mcp は commit diff surface を持つ」 | 当時 github-rag-mcp は `path.endsWith(".md")` のみで commit diff indexing は未実装。PR #1069 を close して実装 #80/#81 を先行させ、その後 #1056/#1070 で spec を実体反映 |
