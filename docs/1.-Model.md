...
Criterion resident, provenance in git（基準は常駐、来歴は git）：State 面への (A) の適用であり、対象は `rules/` と `skills/` の本文。適用の瞬間に消費されるのは基準と現在の状態である。それがどう書かれるに至ったか——どの issue が提起し、どの PR が確定させ、どの commit が動かしたか——は、その瞬間に立っている誰にも消費されない。したがってその来歴を運ぶ番号は常駐すべき state ではなく、例外なく落ちる：issue 番号、PR 番号、commit SHA、およびそれらの形をしたサンプル値。発火するのは基準であって、その基準を生んだ番号は何も発火させない。

到達可能性を手放すのではない。`skills/model-trigger-check-gate-actions/SKILL.md` の Retrieval tools は来歴への問いをすでに別の面へ振り分けている——attribution と timeline は `git log` / `git blame`、過去判断は issue / PR への RAG であり、rule 本文ではない。`docs/` と wiki は来歴を意図して保持する retrieval 面であり、本項の対象外。rule から番号を落としても、番号はもともとそれに答えていた面に立ったままになる。

これは次の二つではない：

- **文の根拠を落としてよいという許可ではない。** 番号を剥がすのは、それが裏付けていた基準文が残る箇所に限る。基準を述べていない文から番号だけ抜けば、背後に何も無い主張が残る。直し方は基準を述べることであって、番号を残すことではない。
...
- provenance-in-text 系（(A)）：issue / PR 番号や commit SHA を `rules/` / `skills/` に書こうとしている（format サンプル中も含む。読んだ時点で生きた参照に見える）／rule の文が、今何を要求するかではなく、その rule がどう書かれるに至ったか（何を置き換えたか、誰が捕まえたか、いつ動いたか）を語っている／常駐 rule が、ルールセット自身が期限切れを保証している transient 成果物を指しており、読み手が辿った瞬間にその参照が解決しない
