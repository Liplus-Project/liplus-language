...
- Cross-reference 整合性アサーション（手順 4 後・手順 5 前）：`{tmpdir}/*.md` 内のすべての wiki 内部リンクが実在 entry を指していることを検証する。抽出前に、本文からコード記法を除去する。コード内、または HTML コメント内のリンク風記法は記法そのものの説明か、描画されない markup であり、実リンクではない。まずブロックレベルのコード記法を除去し、そのあとインラインコードスパンを除去する：
  - フェンスドコードブロック：開始行はバッククォート3個以上、またはチルダ3個以上の run のみで構成される行（同じ行に info string が続くのは許容）。ブロックは、同じ文字の run で長さが開始側以上である次の行（閉じフェンス）まで及ぶ。該当する行が無ければ、フェンスはファイル末尾まで及ぶ。
  - インデントコードブロック：4スペースまたはタブでインデントされた行が連続する最大の範囲。インデントされていない最初の行の手前で終わる。
  - HTML の `<code>` / `<pre>` 要素、および HTML コメント（`<!-- ... -->`）。
  そのあと、インラインコードスパン（バッククォート N 個の run を、同じ N 個の run で閉じた範囲）を除去する。ちょうど N 個で閉じる run が無ければ、開始側の run はコードスパンとみなさず、そのまま literal text として残す。
  ブロックレベルを先に除去することで、フェンス自身のバッククォート run がインラインスパンの区切りと誤認されて無関係なテキストを飲み込むことを防ぐ。解決集合を構築する：
